...
namespace NetSqlAzMan
{
    ...
    public sealed partial class SqlAzManStoreGroup :IAzManStoreGroup
    {
        ...
        public IAzManStoreGroupMember[] GetStoreGroupAllMembers() {
            if (this.groupType != GroupType.Basic)
                throw new InvalidOperationException("Method not supported for LDAP Query Groups");
            ...
            foreach (var row in sgm) {
                ...
                storeGroupAllMembers[index] = new SqlAzManStoreGroupMember(this.db, this, row.StoreGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, this.ens, row.DomainProfile, row.samAccountName, row.cn, row.displayName, row.objectSidString, row.distinguishedName, row.objectClass);
                #endregion
                if (this.ens != null)
        ...
        public IAzManStoreGroupMember[] GetStoreGroupMembers() {
            ...
            foreach (var row in sgm) {
                #region ***PERSONALIZADO***
                storeGroupMembers[index] = new SqlAzManStoreGroupMember(this.db, this, row.StoreGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, this.ens, row.DomainProfile, row.samAccountName, row.cn, row.displayName, row.objectSidString, row.distinguishedName, row.objectClass);
                #endregion
                if (this.ens != null)
        ...
        public IAzManStoreGroupMember GetStoreGroupMember(IAzManSid sid) {
            ...
            if ((sgm = (from t in this.db.StoreGroupMembers()
                        where t.StoreGroupId == this.storeGroupId && t.ObjectSid == sid.BinaryValue
        ...
        public IAzManStoreGroupMember[] GetStoreGroupNonMembers() {
            ...
            foreach (var row in sgnm) {
                #region ***PERSONALIZADO***
                storeGroupNonMembers[index] = new SqlAzManStoreGroupMember(this.db, this, row.StoreGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, this.ens, row.DomainProfile, row.samAccountName, row.cn, row.displayName, row.objectSidString, row.distinguishedName, row.objectClass);
                #endregion
                if (this.ens != null)
