...
namespace NetSqlAzMan {
	...
	public sealed partial class SqlAzManStore : IAzManStore {
		...
		public IAzManApplication GetApplication(string applicationName) {
			...
			if ((app = (from t in this.db.Applications()
							where t.StoreId == this.storeId && t.Name == applicationName
		...
		public IAzManStoreGroup GetStoreGroup(string name) {
			...
			if ((sgr = (from tf in this.db.StoreGroups()
							where tf.Name == name && tf.StoreId == this.storeId
							select tf).FirstOrDefault()) != null) {
		...
		public IAzManStoreGroup GetStoreGroup(IAzManSid sid) {
			StoreGroupsResult sgr;
			if ((sgr = (from t in this.db.StoreGroups()
							where t.ObjectSid == sid.BinaryValue && t.StoreId == this.storeId
							select t).FirstOrDefault()) != null) {
		...
		public IAzManAttribute<IAzManStore> GetAttribute(string key) {
			...
			if ((sar = (from t in this.db.StoreAttributes()
							where t.StoreId == this.storeId && t.AttributeKey == key
							select t).FirstOrDefault()) != null) {
		...
		public IAzManDBUser GetDBUser(IAzManSid customSid) {
			var _users = this.db.GetDBUsers(this.name, null, customSid.BinaryValue, null);
			IAzManDBUser _result;
			if (_users.Count().Equals(0))
				throw SqlAzManException.DBUserNotFoundException(customSid.StringValue, null);
			else {
				SqlAzManStorage _storage = (SqlAzManStorage)this.Storage;
			...
			//else {
			//	SqlAzManStorage _storage = (SqlAzManStorage)this.Storage;
			//	Dictionary<string, object> _customCols = _storage.getCustomColumnsFromDataRow(dtDBUsers.Rows[0]);
		...
		public IAzManDBUser GetDBUser(string userName) {
			var _users = this.db.GetDBUsers(this.name, null, null, userName);
			IAzManDBUser _result;
			if (_users.Count().Equals(0))
		...
		public IAzManDBUser[] GetDBUsers() {
			var _users = this.db.GetDBUsers(this.name, null, null, null);
