...
namespace NetSqlAzMan.WebApiClientHelpers {
	public class BaseHelper<BSO> {
		...
		//Debería ser PRIVATE, refactorizar llamadas externas!
		internal HttpWebApiRequestException getHttpWebApiRequestException(string requestUri, HttpResponseMessage responseMessage) {
			...
			switch (_mediaType) {
				case Global.MimeType_NoContent:
					...
					return new HttpWebApiRequestException("Error al realizar la solicitud web.", getFullUri(requestUri), responseMessage.StatusCode, responseMessage.ReasonPhrase, _viewModelForEmptyContent);
				...
				case Global.MimeType_AppJson:
					...
					if (responseMessage.StatusCode == System.Net.HttpStatusCode.InternalServerError) {
						var _deserializedContent = JsonConvert.DeserializeObject<LDAPHelperDTO.ErrorSummary>(_jsonContent);

						return new HttpWebApiRequestException("Error al realizar la solicitud web.", getFullUri(requestUri), responseMessage.StatusCode, responseMessage.ReasonPhrase, _deserializedContent);
					}
					else { //VBASTIDAS: Esto lo usaba antes en la versión de LdapProxyWebApi que estaa implementado en .NET 4.51
				...
				case Global.MimeType_TextHtml:
					...
					return new HttpWebApiRequestException("Error al realizar la solicitud web.", getFullUri(requestUri), responseMessage.StatusCode, responseMessage.ReasonPhrase, _viewModelForHtml);
