...
namespace NetSqlAzMan.WebApiClientHelpers {
	internal class LdapGroupMembershipHelper<BSO> : BaseHelper<BSO> {
		...
		/// <returns></returns>
		internal async Task<Dictionary<string, IEnumerable<object>>> GetGroupMembership(string profile, bool useGC, LDAPHelperDTO.EntryAttribute attribute, string attributeFilter) {
			var _requestUri = string.Format("api/LdapGroupMembership/?profile={0}&useGC={1}&attribute={2}&attributeFilter={3}", profile, useGC.ToString(), attribute.ToString(), attributeFilter);
