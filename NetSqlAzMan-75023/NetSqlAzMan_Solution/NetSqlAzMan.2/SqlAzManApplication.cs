...
namespace NetSqlAzMan {
	...
	public sealed partial class SqlAzManApplication : IAzManApplication {
		...
		public IAzManItem GetItem(string itemName) {
			ItemsResult items;
			if ((items = (from t in this.db.Items()
							  where t.Name == itemName && t.ApplicationId == this.applicationId
		...
		public IAzManApplicationGroup GetApplicationGroup(string name) {
			...
			if ((agr = (from t in this.db.ApplicationGroups()
							where t.Name == name && t.ApplicationId == this.applicationId
							select t).FirstOrDefault()) != null) {
		...
		public IAzManApplicationGroup GetApplicationGroup(IAzManSid sid) {
			...
			if ((agr = (from t in this.db.ApplicationGroups()
							where t.ObjectSid == sid.BinaryValue && t.ApplicationId == this.applicationId
							select t).FirstOrDefault()) != null) {
		...
		public IAzManAttribute<IAzManApplication> GetAttribute(string key) {
			...
			if ((aa = (from t in this.db.ApplicationAttributes()
						  where t.ApplicationId == this.applicationId && t.AttributeKey == key
						  select t).FirstOrDefault()) != null) {
		...
		public IAzManDBUser GetDBUser(IAzManSid customSid) {
			var _users = this.db.GetDBUsers(this.store.Name, this.name, customSid.BinaryValue, null);
			IAzManDBUser _result;
			if (_users.Count().Equals(0))
				throw SqlAzManException.DBUserNotFoundException(customSid.StringValue, null);
			else {
				SqlAzManStorage _storage = (SqlAzManStorage)this.Store.Storage;
				LINQ.GetDBUsersResult _user = _users.First();
				Dictionary<string, object> _customProperties = _storage.GetCustomPropertiesFromUserData(_user);
		...
		public IAzManDBUser GetDBUser(string userName) {
			var _users = this.db.GetDBUsers(this.store.Name, this.name, null, userName);
			IAzManDBUser _result;
			if (_users.Count().Equals(0))
				...
				SqlAzManStorage _storage = (SqlAzManStorage)this.store.Storage;
				LINQ.GetDBUsersResult _user = _users.First();
				Dictionary<string, object> _customCols = _storage.GetCustomPropertiesFromUserData(_user);
				_result = new SqlAzManDBUser(new SqlAzManSID(_user.DBUserSid.ToArray(), true), _user.DBUserName, _user.FullName, _customCols);
			}
			...
			//else {
			//	SqlAzManStorage _storage = (SqlAzManStorage)this.Store.Storage;
			//	Dictionary<string, object> _customColumns = _storage.getCustomColumnsFromDataRow(dtDBUsers.Rows[0]);
			//	result = new SqlAzManDBUser(dtDBUsers.Rows[0], _customColumns);
			//}
			//return result;
		}
		...
		public IAzManDBUser[] GetDBUsers() {
			var _users = this.db.GetDBUsers(this.store.Name, this.name, null, null);
			...
			//	Dictionary<string, object> _customCols = _storage.getCustomColumnsFromDataRow(row);
			//	result[i++] = new SqlAzManDBUser(row, _customCols);
			//}
			//return result;
		}
		#endregion DB Users
