    ...
    public sealed partial class SqlAzManApplicationGroup :IAzManApplicationGroup, IAzManExport
    {
        ...
        public IAzManApplicationGroupMember[] GetApplicationGroupMembers() {
            ...
            foreach (var row in agm) {
                ...
                applicationGroupMembers[index] = new SqlAzManApplicationGroupMember(this.db, this, row.ApplicationGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, row.DomainProfile, row.samAccountName, row.cn, row.displayName, row.objectSidString, row.distinguishedName, row.objectClass, this.ens);
                #endregion
                if (this.ens != null)
        ...
        public IAzManApplicationGroupMember GetApplicationGroupMember(IAzManSid sid) {
            ...
            if ((agm = (from t in this.db.ApplicationGroupMembers()
                        where t.ApplicationGroupId == this.applicationGroupId && t.ObjectSid == sid.BinaryValue
        ...
        public IAzManApplicationGroupMember[] GetApplicationGroupNonMembers() {
            ...
            foreach (var row in agnm) {
                #region ***PERSONALIZADO***
                applicationGroupNonMembers[index] = new SqlAzManApplicationGroupMember(this.db, this, row.ApplicationGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, row.DomainProfile, row.samAccountName, row.cn, row.displayName, row.objectSidString, row.distinguishedName, row.objectClass, this.ens);
                #endregion
                if (this.ens != null)
        ...
        public IAzManApplicationGroupMember[] GetApplicationGroupAllMembers() {
            ...
            foreach (var row in agam) {
                #region ***PERSONALIZADO***
                applicationGroupAllMembers[index] = new SqlAzManApplicationGroupMember(this.db, this, row.ApplicationGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, row.DomainProfile, row.samAccountName, row.cn, row.displayName, row.objectSidString, row.distinguishedName, row.objectClass, this.ens);
                #endregion
                if (this.ens != null)
        ...
        /// Determines whether [is in group] [the specified windows identity].
        /// </summary>
        /// <param name="azUser">The db user.</param>
        /// <returns>
        /// 	<c>true</c> if [is in group] [the specified windows identity]; otherwise, <c>false</c>.
        /// </returns>
        public bool IsInGroup(IAzManDBUser azUser) {
            if (azUser == null)
                throw new ArgumentNullException("azUser");

            //Se usa el mismo método para evaluar a un usuario de BD 
            //o un usuario LDAP
            bool _flag;
            _flag = this.isAMemberOfGroup(true, this.sid.BinaryValue, this.application.Store.Storage.Mode == NetSqlAzManMode.Developer, DirectoryServicesUtils.rootDsePath, azUser.CustomSid.BinaryValue, 0);
                        ...
                        if (childNode.Name == "ApplicationGroupMember") {
                            ...
                            switch (childNode.Attributes["WhereDefined"].Value) {
                                case "Application":
                                    ...
                                    break;
                                case "LDAP":
                                    whereDefined = WhereDefined.LDAP;
                                    break;
                                case "Local":
                                ...
                                case "Store":
                                    ...
                                    break;
                                case "Database":
                                    whereDefined = WhereDefined.Database;
                                    break;
                                default:
