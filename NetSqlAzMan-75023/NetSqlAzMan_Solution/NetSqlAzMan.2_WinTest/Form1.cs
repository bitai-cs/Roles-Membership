...
namespace NetSqlAzMan_WinTest
{
	...
	{
		private NetSqlAzMan.SqlAzManStorage _storage = Program.Storage;
		private NetSqlAzMan.SqlAzManStore _store;
		private NetSqlAzMan.SqlAzManApplication _application;

		private IAzManSid currentOwnerSid = new SqlAzManSID(System.Security.Principal.WindowsIdentity.GetCurrent().User.Value);
		public Form1() {
			InitializeComponent();

			sqlAzManStoreBindingSource.DataSource = _storage.Stores.Values;
		}
...
";
		...
		private void btnDBGetUsers_Click(object sender, EventArgs e) {
			...
			IAzManStorage storage = new SqlAzManStorage(Program.CONFIG_ConnectionString);
			IAzManStore store = storage["SISE"];
			IAzManDBUser dbu = store.GetDBUser("ADMIN");

			MessageBox.Show(this, dbu.CustomColumns["BranchOfficeIds"].ToString() + "\n\r" + dbu.CustomColumns["BranchOfficeNames"].ToString(), dbu.UserName, MessageBoxButtons.OK, MessageBoxIcon.Information);

			MessageBox.Show(this, dbu.CustomColumns["DepartmentId"].ToString() + "\n\r" + dbu.CustomColumns["DepartmentName"].ToString(), dbu.UserName, MessageBoxButtons.OK, MessageBoxIcon.Information);

			IAzManDBUser[] dbUsers = store.GetDBUsers();
		...
		private void btnCheckAccessTest_Click(object sender, EventArgs e) {
			IAzManStorage storage = new SqlAzManStorage("data source=172.22.3.177\\sql2k14,1434;Initial Catalog=AzMan;User id=secman;password=sisepuede;");
			List<KeyValuePair<string, string>> attributes;
			var auth = storage.CheckAccess("SISE", "LOGIN", "IniciarSesion", WindowsIdentity.GetCurrent(), DateTime.Now, false, out attributes);
			MessageBox.Show(auth.ToString());
			return;
		...
		private void btnCacheServiceTest_Click(object sender, EventArgs e) {
			...
			//      var auth = c.CheckAccessForWindowsUsersWithAttributesRetrieve(out attributes, "SISE", "TRAMITES_ALUMNADO", "Sesion", user, groups, DateTime.Now, false, null);
			//      c.Close();
			//      TimeSpan ts = DateTime.Now.Subtract(now);
			//      if (auth != sr.AuthorizationType.Allow) {
		...
		private void button1_Click_3(object sender, EventArgs e) {
			IAzManStorage _storage = new SqlAzManStorage(Program.CONFIG_ConnectionString);
			IAzManDBUser _dbUser;
			Exception _hex;
			if (!_storage.GetAzManDBUser("ADMIN", out _dbUser, out _hex)) {
				MessageBox.Show(_hex.Message);
				return;
			}

			var sb = new StringBuilder();
			sb.AppendLine("UserID: " + _dbUser.CustomSid.StringValue);
			sb.AppendLine("Username: " + _dbUser.UserName);
			sb.AppendLine("Display Name: " + _dbUser.DisplayName);
			sb.AppendLine("Custom Columns:");
			foreach (var _k in _dbUser.CustomColumns) {
				sb.AppendLine(string.Format("\t{0}: {1}", _k.Key, _k.Value.ToString()));
			}
			sb.AppendLine();
		...
		private void button2_Click(object sender, EventArgs e) {
			try {
				IAzManStorage storage = new SqlAzManStorage(Program.CONFIG_ConnectionString);
				IAzManDBUser user = storage.GetDBUser("ADMIN");
				AuthorizationType aut;

				aut = storage.CheckAccess("LTA", "APP", "IniciarSesion", user, DateTime.Now, false, null);

				MessageBox.Show(aut.ToString());
			}
			catch (Exception ex) {
		...
		private async void butnChangeDBUserPassword_Click(object sender, EventArgs e) {
			Exception _hex = null;

			try {
				//SqlAzManStorage _storage = new SqlAzManStorage(CONFIG_ConnectionString);

				//if (!_storage.ChangeDBUserPassword("VBASTIDAS", "456123", "17011981", "123232", out _hex))
				//	MessageBox.Show(this, _hex.Message + Environment.NewLine + _hex.StackTrace, this.Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
				//else
				//	MessageBox.Show(this, "Contraseña cambiada.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Error);

				var _fac = new NetSqlAzMan.CustomBussinessLogic.DBUserBusinessFactory();
				var _user = await _fac.GetByUsernameAsync("ADMIN");

				if (_user == null)
					throw new Exception("No existe el usuario!");

				_user.CurrentPassword = "17011981V1k0";
				_user.NewPassword = "17011981";
				_user.PasswordConfirmation = "17011981";

				var _r = await _fac.ChangePasswordAsync(_user);

				MessageBox.Show(this, "Se cambió la constraseña ", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
			catch (Exception ex) {
				MessageBox.Show(this, ex.Message + Environment.NewLine + ex.StackTrace, ex.Source, MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}

		private void button3_Click(object sender, EventArgs e) {
			try {
				SqlAzManStorage _storage = new SqlAzManStorage(Program.CONFIG_ConnectionString);

				IAzManDBUser _azManUser;
				Exception _hex;
				bool _result;
				AuthorizationType _auth;

				_result = _storage.GetLDAPUser("LANPERU", "usr_ext01", out _azManUser, out _hex);
				if (!_result) {
					textBox1.Text = getAllExceptions(_hex);
					return;
				}

				_auth = _storage.CheckAccessLDAP("LTA", "APP", "IniciarSesion", _azManUser.DomainProfile, _azManUser.UserName, _azManUser.CustomSid.StringValue, DateTime.Now, false);

				MessageBox.Show(_azManUser.UserName + " -> " + _auth.ToString());

				_result = _storage.GetLDAPUser("LANCHILE", "atordeur", out _azManUser, out _hex);
				if (!_result) {
					textBox1.Text = getAllExceptions(_hex);
					return;
				}
		...
		private void button4_Click(object sender, EventArgs e) {
			...
			IAzManDBUser _azManUser;
			Exception _hex;
			var _result = _storage.GetLDAPUser("LANPERU", "usr_ext01", out _azManUser, out _hex);

			if (_result)
				textBox1.Text = _azManUser.CustomSid.StringValue + Environment.NewLine + _azManUser.UserName + Environment.NewLine +
					_azManUser.DisplayName + Environment.NewLine;
			else {
				textBox1.Text = getAllExceptions(_hex);
			}
		}

		private string getAllExceptions(Exception hex) {
			string _all = null;

			NetSqlAzMan.CustomBussinessLogic.LdapWebApiClientHelpers.HttpWebApiRequestException _wex = null;
			if (hex.GetType() == typeof(NetSqlAzMan.CustomBussinessLogic.LdapWebApiClientHelpers.HttpWebApiRequestException)) {
				_wex = hex as NetSqlAzMan.CustomBussinessLogic.LdapWebApiClientHelpers.HttpWebApiRequestException;

				_all = _wex.GetType().FullName + ": " + _wex.Message + Environment.NewLine +
					"Request: " + _wex.RequestUri + Environment.NewLine +
					((int)_wex.StatusCode).ToString() + ": " + _wex.ReasonPhrase + Environment.NewLine + _wex.StackTrace + Environment.NewLine + Environment.NewLine;

				if (_wex.ResponseContent.GetType() == typeof(NetSqlAzMan.CustomBussinessLogic.LdapWebApiClientHelpers.Models.TextHtmlContentErrorResponse)) {
					NetSqlAzMan.CustomBussinessLogic.LdapWebApiClientHelpers.Models.TextHtmlContentErrorResponse _current = (NetSqlAzMan.CustomBussinessLogic.LdapWebApiClientHelpers.Models.TextHtmlContentErrorResponse)_wex.ResponseContent;
					_all += _current.HTMLText;
				}
				else if (_wex.ResponseContent.GetType() == typeof(LDAPHelperDTO.ErrorSummary)) {
					ErrorSummary _current = (ErrorSummary)_wex.ResponseContent;
					while (_current != null) {
						_all += _current.GetType().Name + ": " + _current.ErrorType + " - " + _current.Source + " - " + _current.ErrorDescription + Environment.NewLine;
						foreach (var _i in _current.ErrorDetail) {
							_all += _i + Environment.NewLine;
						}
						_all += _current.StackTrace + Environment.NewLine + Environment.NewLine;
						_current = _current.InnerError;
					}
				}
				else {
				...
				Exception _current = hex;
				while (_current != null) {
					_all += _current.GetType().Name + ": " + _current.Message + Environment.NewLine + _current.StackTrace + Environment.NewLine + Environment.NewLine;
		...
		private void button5_Click(object sender, EventArgs e) {
			try {
				...
				//string _status;
				//string _stackTrace;
				_azManUser = (SqlAzManDBUser)_storage.GetDBUser("admcampus");
				textBox1.Text = _azManUser.UserName + Environment.NewLine +
					_azManUser.DisplayName + Environment.NewLine + _azManUser.CustomSid.StringValue + Environment.NewLine + _azManUser.CustomColumns.Count.ToString();
			}
			catch (Exception ex) {
		...
		private void button6_Click(object sender, EventArgs e) {
			var _p = new NetSqlAzMan.CustomBussinessLogic.PasswordHasher();
			txtbHashed.Text = _p.HashPassword(txtbTextToHash.Text);
		}

		private void button7_Click(object sender, EventArgs e) {
			var _p = new NetSqlAzMan.CustomBussinessLogic.PasswordHasher();
			var _r = _p.VerifyHashedPassword(txtbHashToVerify.Text, txtbTextToHash.Text);

			MessageBox.Show(_r.ToString());
		}

		private void button9_Click(object sender, EventArgs e) {
			SqlAzManStorage _storage = new SqlAzManStorage(Program.CONFIG_ConnectionString);
			IAzManDBUser _azUser = null;
			Exception _hex = null;
			var _result = _storage.GetLDAPUser("lanperu", "usr_ext01", out _azUser, out _hex);

			if (!_result) {
				MessageBox.Show(_hex.Message + Environment.NewLine + _hex.StackTrace);
				return;
			}

			var _app = _storage.Stores["LTA"].Applications["APP"];
			var _roles = _app.GetItems(ItemType.Role);

			string _roleList = null;
			foreach (var _role in _roles) {
				var _auth = _storage.CheckAccessLDAP("LTA", "APP", _role.Name, _azUser.DomainProfile, _azUser.UserName, _azUser.CustomSid.StringValue, DateTime.MaxValue, false);

				_roleList += _role.Name + ": " + _auth.ToString() + Environment.NewLine;
			}

			MessageBox.Show(_roleList);
		}

		private void button10_Click(object sender, EventArgs e) {
			try {
				var storage = new SqlAzManStorage(Program.CONFIG_ConnectionString);
				//IAzManDBUser user = storage.GetDBUser("ADMIN");
				AuthorizationType aut;

				var _winUser = WindowsIdentity.GetCurrent();

				aut = storage.CheckAccess("LTA", "APP", "IniciarSesion", _winUser, DateTime.Now, false);

				MessageBox.Show(_winUser.Name + " -> " + aut.ToString());
			}
			catch (Exception ex) {
				MessageBox.Show(ex.Message, ex.Source, MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		...
		private void combStore_SelectedValueChanged(object sender, EventArgs e) {
			if (combStore.SelectedItem != null) {
				_store = (NetSqlAzMan.SqlAzManStore)combStore.SelectedItem;
				sqlAzManApplicationBindingSource.DataSource = _store.Applications.Values;
			}
		}

		private void button11_Click(object sender, EventArgs e) {
			var _items = _application.GetItems();
			//var _json = JsonConvert.SerializeObject(_items);
			//txtbItemsJson.Text = _json;
		}

		private void combApplication_SelectedValueChanged(object sender, EventArgs e) {
			if (combApplication.SelectedItem != null) {
				_application = (NetSqlAzMan.SqlAzManApplication)combApplication.SelectedItem;
			}
		}
		...
		private void button12_Click(object sender, EventArgs e) {
			...
			var _result = _storage.GetAzManDBUser("vbastidas", out _azUser, out _hex);
			if (!_result) {
				MessageBox.Show(_hex.Message + Environment.NewLine + _hex.StackTrace);
				return;
			}

			var _auth = _storage.CheckAccess(_store.Name, _application.Name, "IniciarSesion", _azUser, DateTime.Now, false);

			MessageBox.Show(_auth.ToString());
		}

		private void button13_Click(object sender, EventArgs e) {
			List<KeyValuePair<string, object>> _list = new List<KeyValuePair<string, object>>() {
				new KeyValuePair<string, object>("p1","user"),
				new KeyValuePair<string, object>("p2",DateTime.Now),
				new KeyValuePair<string, object>("p3", 345)
			};
