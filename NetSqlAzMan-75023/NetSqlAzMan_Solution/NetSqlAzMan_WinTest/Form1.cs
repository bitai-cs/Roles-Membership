...
";
		...
		private void btnDBGetUsers_Click(object sender, EventArgs e) {
			...
			IAzManStorage storage = new SqlAzManStorage("data source=172.22.1.12;Initial Catalog=SECU_STOR;Integrated Security = SSPI;");
		...
		private void button1_Click_3(object sender, EventArgs e) {
			NetSqlAzMan.NetSqlAzManWCFCacheService.CacheServiceClient c = new NetSqlAzMan.NetSqlAzManWCFCacheService.CacheServiceClient("WSHttpBinding_ICacheService");
			var aut = c.CheckAccessForDatabaseUsersWithoutAttributesRetrieve("SISE", "TRAMITES_ALUMNADO", "Sesion", "ADMIN", DateTime.Now, true, null);
			MessageBox.Show(aut.ToString());
			//StringBuilder sb = new StringBuilder();

			//sr.CacheServiceClient c = new sr.CacheServiceClient();

			//sr.DBUser u = c.GetDBUser("ADMIN");

			//sb.AppendLine("UserID: " + u.UserID.ToString());
			//sb.AppendLine("Username: " + u.UserName);
			//sb.AppendLine("AttributeString: " + u.AttributeString);
			//sb.AppendLine();
			//sb.AppendLine();

			//string password = "17011981";
			//bool boolValido = c.ValidatePassword("ADMIN", password);

			//sb.AppendLine("Password " + password + " valido? " + boolValido.ToString());
			//sb.AppendLine();
			//sb.AppendLine();

			//System.Diagnostics.Debugger.Break();

			////u.AttributeString.Split(Convert.ToChar(3)).
		...
		private void button2_Click(object sender, EventArgs e) {
			try {
				...
				aut = storage.CheckAccess("LTA", "APP", "IniciarSesion", user, DateTime.Now, false);

				MessageBox.Show(aut.ToString());

				aut = storage.CheckAccess("LTA", "APP", "IniciarSesion", WindowsIdentity.GetCurrent(), DateTime.Now, false);

				MessageBox.Show(WindowsIdentity.GetCurrent().User.Value + " -> " + aut.ToString());
			}
			catch (Exception ex) {
		...
		private void butnChangeDBUserPassword_Click(object sender, EventArgs e) {
			string _statusMessage;

			try {
				IAzManStorage _storage = new SqlAzManStorage("data source=172.45.12.37\\.sqlexpress; Initial Catalog=AzMan; User Id=secman; Password=sisepuede");

				IAzManDBUser _user = _storage.GetDBUser("admin");

				//if (!_storage.ChangeDBUserPassword(_user, "456123", "456123", "456123", out _statusMessage))
				//	MessageBox.Show(this, _statusMessage, this.Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
				//else
		...
		private void button3_Click(object sender, EventArgs e) {
			try {
				//NetSqlAzMan.LdapHelperSvcRef.LdapHelperClient _c = new NetSqlAzMan.LdapHelperSvcRef.LdapHelperClient("UniversidadServer_BasicHttpBinding");
				//NetSqlAzMan.LdapHelperSvcRef.LDAPSearchResult[] _r = _c.Search("UNIVERSIDAD", "vbastidas", "17011981V1c0", NetSqlAzMan.LdapHelperSvcRef.LdapHelperADProperties.samAccountName, "vbastidas");

				//NetSqlAzMan.LdapHelperSvcRef.LDAPSearchResult _obj = _r[0];

				//IAzManStorage storage = new SqlAzManStorage("data source=172.22.3.177\\sql2k14,1434;Initial Catalog=AzMan;User id=secman;password=sisepuede;");
				//AuthorizationType _auth = storage.CheckAccessLDAP("SISE", "LOGIN", "IniciarSesion", _obj.LDAPDomain, _obj.samAccountName, _obj.objectSid, DateTime.Now, false);
		...
		private void button4_Click(object sender, EventArgs e) {
			var _storage = new SqlAzManStorage(Program.CONFIG_ConnectionString);
			IAzManDBUser _azManUser;
			string _statusType;
			string _status;
			string _stackTrace;
			var _result = _storage.GetLDAPUser("sise", "vbastidas", out _azManUser, out _statusType, out _status, out _stackTrace);
			if (_result)
				textBox1.Text = _azManUser.UserName + Environment.NewLine +
					_azManUser.DisplayName + Environment.NewLine + _azManUser.CustomSid.StringValue;
			else
				textBox1.Text = _status + Environment.NewLine + _stackTrace;

		}

		private void button5_Click(object sender, EventArgs e) {
			try {
				SqlAzManStorage _storage = new SqlAzManStorage("data source=172.22.3.177\\sql2k14,1434;Initial Catalog=AzMan;User id=secman;password=sisepuede;");
