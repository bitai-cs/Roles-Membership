...
namespace NetSqlAzMan.DirectoryServices {
	...
	public static class DirectoryServicesUtils {
		...
		static DirectoryServicesUtils() {
			try {
				var _cfg = System.Configuration.ConfigurationManager.AppSettings["RootDSEPath"];
				var _usr = System.Configuration.ConfigurationManager.AppSettings["Active Directory LookUp Username"];
				var _pwd = System.Configuration.ConfigurationManager.AppSettings["Active Directory LookUp Password"];
				if (!string.IsNullOrEmpty(_cfg)) {
					DirectoryServicesUtils.rootDsePath = _cfg;
					DirectoryServicesUtils.userName = _usr;
					DirectoryServicesUtils.password = _pwd;
					return;
				}
		...
		public static SearchResultCollection ExecuteLDAPQuery(string lDapQuery) {
			try {
				...
				root.RefreshCache();
				DirectorySearcher searcher = new DirectorySearcher(root, DirectoryServicesUtils.GetLDAPQueryPart(lDapQuery), new string[] { "objectSid", "samAccountName", "displayName", "objectCategory", "objectSidBytes" });
