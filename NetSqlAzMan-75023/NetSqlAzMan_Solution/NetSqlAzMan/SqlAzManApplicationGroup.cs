	...
	public sealed partial class SqlAzManApplicationGroup : IAzManApplicationGroup, IAzManExport
	{
		...
		public IAzManApplicationGroupMember[] GetApplicationGroupMembers() {
			...
			foreach (var row in agm) {
				...
				applicationGroupMembers[index] = new SqlAzManApplicationGroupMember(this.db, this, row.ApplicationGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value,row.LDAPDomain, this.ens);
				#endregion
				if (this.ens != null)
		...
		public IAzManApplicationGroupMember[] GetApplicationGroupAllMembers() {
			...
			foreach (var row in agam) {
				#region ***PERSONALIZADO***
				applicationGroupAllMembers[index] = new SqlAzManApplicationGroupMember(this.db, this, row.ApplicationGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, row.LDAPDomain, this.ens);
				#endregion
				if (this.ens != null)
