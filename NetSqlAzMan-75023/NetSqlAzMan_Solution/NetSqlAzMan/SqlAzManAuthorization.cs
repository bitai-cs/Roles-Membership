...
namespace NetSqlAzMan {
	...
	public sealed partial class SqlAzManAuthorization : IAzManAuthorization {
		...
		public IAzManAttribute<IAzManAuthorization> GetAttribute(string key) {
			...
			if ((attr = (from t in this.db.AuthorizationAttributes()
							 where t.AuthorizationId == this.authorizationId && t.AttributeKey == key
			...
			switch (this.SidWhereDefined) {
				...
				case WhereDefined.LDAP:
					...
					if (!string.IsNullOrEmpty(this.LDAPDomain)) {
						string _endPoint = LDAPServices.LDAPConfigUtils.GetWebSvcEndpointByDomainProfile(this.item.Application.Store.Storage.ConnectionString, this.LDAPDomain);
						LDAPHelperSvcRef.LDAPHelperClient _wsc = new LDAPHelperSvcRef.LDAPHelperClient(_endPoint);
						LDAPHelperSvcRef.LDAPSearchResult[] _sr;
						LDAPHelperSvcRef.StatusInfo _si;
						try {
							if (_wsc.SearchBySid(out _sr, out _si, this.LDAPDomain, this.SID.StringValue, false)) {
								if (!string.IsNullOrEmpty(_sr[0].displayName))
									displayName = string.Format("{0} ({1}\\{2})", _sr[0].displayName, _sr[0].DomainProfile, _sr[0].samAccountName);
								else
									displayName = string.Format("{0}\\{1}", _sr[0].DomainProfile, _sr[0].samAccountName);

								return _sr[0].objectCategory.ToLower().StartsWith("cn=group") ? MemberType.WindowsNTGroup : MemberType.WindowsNTUser;
							}
							else {
								displayName = string.Format("{0}\\{1} [{2}]", this.LDAPDomain, this.SID.StringValue, _si.ExceptionType);
								return MemberType.AnonymousSID;
							}
						}
						catch (Exception ex) {
							displayName = string.Format("{0}\\{1} [{2}]", this.LDAPDomain, this.SID.StringValue, ex.GetType().Name);
							return MemberType.AnonymousSID;
						}
					}
					else {
