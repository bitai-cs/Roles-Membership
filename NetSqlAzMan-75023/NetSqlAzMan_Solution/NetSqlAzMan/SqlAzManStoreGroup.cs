	...
	public sealed partial class SqlAzManStoreGroup : IAzManStoreGroup
	{
		...
		public IAzManStoreGroupMember[] GetStoreGroupAllMembers() {
			...
			foreach (var row in sgm) {
				...
				storeGroupAllMembers[index] = new SqlAzManStoreGroupMember(this.db, this, row.StoreGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, this.ens, row.LDAPDomain);
				#endregion
				if (this.ens != null)
		...
		public IAzManStoreGroupMember[] GetStoreGroupMembers() {
			...
			foreach (var row in sgm) {
				#region ***PERSONALIZADO***
				storeGroupMembers[index] = new SqlAzManStoreGroupMember(this.db, this, row.StoreGroupMemberId.Value, new SqlAzManSID(row.ObjectSid.ToArray(), row.WhereDefined == (byte)(WhereDefined.Database)), (WhereDefined)row.WhereDefined, row.IsMember.Value, this.ens, row.LDAPDomain);
				#endregion
				if (this.ens != null)
