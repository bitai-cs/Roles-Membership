...
EXEC dbo.sp_grantdbaccess @loginame = N'BUILTIN\Administrators',
   @name_in_db = N'BUILTIN\Administrators'
GO
...
IF CHARINDEX('[RootDSE:', @LDAPROOTDSEPART) = 1 
   BEGIN
      SET @LDAPROOTDSEPART = SUBSTRING(@LDAPROOTDSEPART, 10,
                                       CHARINDEX(']', @LDAPROOTDSEPART) - 10)
      SET @LDAPQUERYPART = SUBSTRING(@LDAPQUERY,
                                     CHARINDEX(']', @LDAPQUERY) + 1, 4000)
   END
ELSE 
   BEGIN
...
SET @QUERY = CHAR(39) + '<' + 'LDAP://' + @LDAPROOTDSEPART
   ...
   + @LDAPQUERYPART + ');objectSid;subtree' + CHAR(39) 
...
(
 ...
 CONSTRAINT [PK_UsersDemo] PRIMARY KEY CLUSTERED ([UserID] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
 ...
 CONSTRAINT [PK_ItemsHierarchy] PRIMARY KEY CLUSTERED
   ([ItemId] ASC, [MemberOfItemId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
SELECT DISTINCT
         ...
         END AS IsSqlRole
FROM     #temptable
WHERE    MemberName NOT IN ('NetSqlAzMan_Administrators',
                            'NetSqlAzMan_Managers', 'NetSqlAzMan_Users',
                            'NetSqlAzMan_Readers')
...
(
 ...
 CONSTRAINT [PK_AuthorizationAttributes] PRIMARY KEY CLUSTERED
   ([AuthorizationAttributeId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
 ...
 CONSTRAINT [PK_GroupMembers] PRIMARY KEY CLUSTERED
   ([ApplicationGroupMemberId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
 ...
 CONSTRAINT [PK_ApplicationAttributes] PRIMARY KEY CLUSTERED
   ([ApplicationAttributeId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
 ...
 CONSTRAINT [PK_ApplicationPermissions] PRIMARY KEY CLUSTERED
   ([ApplicationPermissionId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
 ...
 CONSTRAINT [PK_StoreGroupMembers] PRIMARY KEY CLUSTERED
   ([StoreGroupMemberId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
 ...
 CONSTRAINT [PK_StoreAttributes] PRIMARY KEY CLUSTERED
   ([StoreAttributeId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
 ...
 CONSTRAINT [PK_StorePermissions] PRIMARY KEY CLUSTERED
   ([StorePermissionId] ASC) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
RETURN
   ...
   FROM     dbo.[netsqlazman_StoresTable]
   WHERE    dbo.[netsqlazman_CheckStorePermissions]([netsqlazman_StoresTable].StoreId,
                                                    0) = 1
GO
...
IF EXISTS ( SELECT   ApplicationId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@Original_ApplicationId,
                                                     2) = 1 
            ...
            ELSE 
               BEGIN
                  ...
                  ELSE 
                     SET @NotMemberType = 0
               END
            EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] @NotMemberType,
               @MemberObjectSid, @NETSQLAZMANMODE, @LDAPPATH,
...
ELSE 
   IF @GROUPTYPE = 1
      ...
      BEGIN
         EXEC dbo.[netsqlazman_ExecuteLDAPQuery] @LDAPPATH, @LDAPQUERY,
            @sub_members_cur OUTPUT
         FETCH NEXT FROM @sub_members_cur INTO @OBJECTSID
         WHILE @@FETCH_STATUS = 0 
            BEGIN
            ...
            IF (@SidWhereDefined = 4) --Database
               BEGIN
                  SET @Name = (SELECT  DBUserName
                               FROM    dbo.[netsqlazman_GetDBUsers](@StoreName,
                                                              @ApplicationName,
                                                              @sid, NULL))
               END
   IF (@Name IS NULL) 
...
IF EXISTS ( SELECT   ItemId
   ...
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ItemsHierarchyTable]
               ([ItemId], [MemberOfItemId])
      VALUES   (@ItemId, @MemberOfItemId)
      RETURN SCOPE_IDENTITY()
   END
ELSE 
   RAISERROR ('Application permission denied.', 16, 1)
            ...
            ELSE 
               BEGIN
                  ...
                  ELSE 
                     SET @NotMemberType = 0
               END
            EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] @NotMemberType,
               @MemberObjectSid, @NETSQLAZMANMODE, @LDAPPATH,
               @sub_members_cur OUTPUT
            FETCH NEXT FROM @sub_members_cur INTO @OBJECTSID
            WHILE @@FETCH_STATUS = 0 
...
WHILE @@FETCH_STATUS = 0 
   BEGIN
	-- Recursive Call
      EXEC dbo.[netsqlazman_CheckAccess] @PARENTITEMID, @USERSID, @VALIDFOR,
         @LDAPPATH, @PARENTRESULT OUTPUT, @NETSQLAZMANMODE,
         @RETRIEVEATTRIBUTES
      SELECT   @AUTHORIZATION_TYPE = dbo.[netsqlazman_MergeAuthorizations](@AUTHORIZATION_TYPE,
                                                              @PARENTRESULT)
      FETCH NEXT FROM ItemsWhereIAmAMember_cur INTO @PARENTITEMID
   END
...
--store groups members of type 'non-member'
            ...
            EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] 0, @GROUPOBJECTSID,
               @NETSQLAZMANMODE, @LDAPPATH, @members_cur OUTPUT -- non-members
	...
	-- user is a non-member
            ...
            IF @ISMEMBER = 1 
               BEGIN
                  ...
                  EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] 1,
                     @GROUPOBJECTSID, @NETSQLAZMANMODE, @LDAPPATH,
	...
	--application groups members of type 'non-member'
               ...
               EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] 0,
                  @GROUPOBJECTSID, @NETSQLAZMANMODE, @LDAPPATH,
               ...
               IF @ISMEMBER = 1 
                  BEGIN
                     ...
                     EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] 1,
                        @GROUPOBJECTSID, @NETSQLAZMANMODE, @LDAPPATH,
                        @members_cur OUTPUT -- members
                     FETCH NEXT FROM @members_cur INTO @OBJECTSID
                     WHILE @@FETCH_STATUS = 0 
...
IF @GROUPTYPE = 0 -- STORE GROUP
   EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] 0, @GROUPOBJECTSID,
      @NETSQLAZMANMODE, @LDAPPATH, @member_cur OUTPUT
ELSE -- APPLICATON GROUP
   EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] 0, @GROUPOBJECTSID,
      @NETSQLAZMANMODE, @LDAPPATH, @member_cur OUTPUT
...
IF @GROUPTYPE = 0 -- STORE GROUP
   EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] 1, @GROUPOBJECTSID,
      @NETSQLAZMANMODE, @LDAPPATH, @member_cur OUTPUT
ELSE -- APPLICATON GROUP
   EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] 1, @GROUPOBJECTSID,
      @NETSQLAZMANMODE, @LDAPPATH, @member_cur OUTPUT
...
EXEC dbo.[netsqlazman_CheckAccess] @ITEMID, @USERSID, @VALIDFOR, @LDAPPATH,
   @AUTHORIZATION_TYPE OUTPUT, @NETSQLAZMANMODE, @RETRIEVEATTRIBUTES
SELECT   *
...
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Readers',
   @membername = N'BUILTIN\Administrators'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Readers',
   @membername = N'NetSqlAzMan_Administrators'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Readers',
   @membername = N'NetSqlAzMan_Managers'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Readers',
   @membername = N'NetSqlAzMan_Users'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Users',
   @membername = N'BUILTIN\Administrators'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Users',
   @membername = N'NetSqlAzMan_Administrators'
...
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Users',
   @membername = N'NetSqlAzMan_Managers'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Managers',
   @membername = N'BUILTIN\Administrators'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Managers',
   @membername = N'NetSqlAzMan_Administrators'
GO
EXEC dbo.sp_addrolemember @rolename = N'NetSqlAzMan_Administrators',
   @membername = N'BUILTIN\Administrators'
GO
