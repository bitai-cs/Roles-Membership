...
namespace AzManWinUI.AzManWebApiClientHelpers
{
	...
	{
		internal AzManApplicationsHelper(string webApiUri) : base(webApiUri) {
		}

		internal async Task<Dictionary<string, IEnumerable<object>>> GetAsync(string store, bool loadAttributes, bool loadItems, bool loadApplicationGroups) {
		...
		//internal async Task<Dictionary<string, IEnumerable<object>>> GetListAsync(string sortOrderField, bool ascOrder, string userFilter, string nameFilter, Nullable<bool> enabledFilter) {
		...
		//	using (var _client = Global.GetHttpClient(true, Global.AcceptHeaderType.ApplicationJson)) {
		...
		//	return _return;
		//}
		internal async Task<Dictionary<string, IEnumerable<object>>> PostAsync(NetSqlAzMan.ServiceBusinessObjects.AzManApplication newApplication, string store) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> PutAsync(string application, string store, NetSqlAzMan.ServiceBusinessObjects.AzManApplication modifiedApplication) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> DeleteAsync(string application, string store) {
			string _requestUri = string.Format("api/AzManApplications/{0}?store={1}", application, store);
			using (var _c = Global.GetHttpClient(this.WebApiUri, true, Global.AcceptHeaderType.ApplicationJson)) {
