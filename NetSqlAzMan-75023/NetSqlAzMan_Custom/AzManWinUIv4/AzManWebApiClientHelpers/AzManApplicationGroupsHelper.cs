...
namespace AzManWinUI.AzManWebApiClientHelpers
{
	...
	{
		#region Constructor
		internal AzManApplicationGroupsHelper(string webApiUri) : base(webApiUri) {
		}
		#endregion

		internal async Task<Dictionary<string, IEnumerable<object>>> GetAllAsync(string store, string application, bool loadApplicationGroupMembers) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetByNameAsync(string id, string store, string application, bool loadApplicationGroupMembers) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetByApplicationGroupIdAsync(string store, string application, string applicationGroupBinaryId, string applicationGroupStringId, bool loadApplicationGroupMembers) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetLdapQueryResultAsync(string store, string application, string applicationGroupName, string ldapQuery) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetIsApplicationGroupMemberAsync(string store, string application, string applicationGroup, string userNameToCheck, string domainProfile = null) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> PostAsync(BSO newBso) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> PutAsync(string id, BSO modifiedBso) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> DeleteAsync(string id, BSO bso) {
			string _requestUri = string.Format("api/AzManApplicationGroups/{0}", id);
			using (var _c = Global.GetHttpClient(this.WebApiUri, true)) {
				...
				//_request.Content = new StringContent(JsonConvert.SerializeObject(applicationGroup), System.Text.Encoding.UTF8, Global.MimeType_AppJson);
				//Send request...
