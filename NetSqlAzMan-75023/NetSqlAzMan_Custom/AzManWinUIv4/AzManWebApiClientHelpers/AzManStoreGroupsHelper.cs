...
namespace AzManWinUI.AzManWebApiClientHelpers
{
	...
	{
		internal AzManStoreGroupsHelper(string webApiUri) : base(webApiUri) {
		}

		internal async Task<Dictionary<string, IEnumerable<object>>> GetAllAsync(string store, bool loadStoreGroupMembers) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetByNameAsync(string id, string store, bool loadStoreGroupMembers) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetBySidAsync(string store, string storeGroupBinaryId, string storeGroupStringId, bool loadStoreGroupMembers) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetLDAPQueryResultAsync(string store, string storeGroupName, string ldapQuery) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetIsStoreGroupMemberAsync(string store, string storeGroup, string userNameToCheck, string domainProfile = null) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> PostAsync(NetSqlAzMan.ServiceBusinessObjects.AzManStoreGroup newBso) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> PutAsync(string id, NetSqlAzMan.ServiceBusinessObjects.AzManStoreGroup storeGroup) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> DeleteAsync(string id, BSO storeGroup) {
			string _requestUri = string.Format("api/AzManStoreGroups/{0}", id);
			using (var _c = Global.GetHttpClient(this.WebApiUri, true)) {
				...
				//_request.Content = new StringContent(JsonConvert.SerializeObject(storeGroup), System.Text.Encoding.UTF8, Global.MimeType_AppJson);
