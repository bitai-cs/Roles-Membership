...
namespace AzManWinUI.AzManWebApiClientHelpers {
	public class DBUsersHelper<BSO> : BaseHelper<BSO> {
		internal DBUsersHelper(string webApiUri) : base(webApiUri) {
		}

		internal async Task<Dictionary<string, IEnumerable<object>>> GetListAsync(string sortOrderField = "UserName", bool ascOrder = true, string userFilter = null, string nameFilter = null, Nullable<bool> enabledFilter = null) {
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> GetByIdAsync(int id) {
			var _return = new Dictionary<string, IEnumerable<object>>();
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> PostAsync(NetSqlAzMan.ServiceBusinessObjects.DBUser data) {
			var _return = new Dictionary<string, IEnumerable<object>>();
		...
		internal async Task<Dictionary<string, IEnumerable<object>>> PutAsync(int id, NetSqlAzMan.ServiceBusinessObjects.DBUser data) {
			var _return = new Dictionary<string, IEnumerable<object>>();
