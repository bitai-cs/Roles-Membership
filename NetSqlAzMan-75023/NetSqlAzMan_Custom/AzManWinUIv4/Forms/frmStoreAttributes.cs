...
namespace NetSqlAzMan.SnapIn.Forms
{
	public partial class frmStoreAttributes : frmBase
	{
		...
		public frmStoreAttributes(string wau)
		{
			InitializeComponent();
		...
		//	return lvi;
		//}
		private ListViewItem CreateStoreAttributeListViewItem(ServiceBusinessObjects.AzManStoreAttribute Attribute)
		{
			ListViewItem lvi = new ListViewItem();
			lvi.Tag = Attribute;
			lvi.ImageIndex = 0;
		...
		private void refreshStoreAttributes()
		{
			...
			var storeAttributes = this._store.Attributes;
			foreach (var storeAttribute in storeAttributes)
			{
		...
		private ServiceBusinessObjects.AzManStoreAttribute FindAttribute(IEnumerable<ServiceBusinessObjects.AzManStoreAttribute> attributes, string key)
		{
			foreach (ServiceBusinessObjects.AzManStoreAttribute a in attributes)
			{
				if (a.Key == key)
					return a;
			}
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			...
			if (dr == DialogResult.OK)
			{
				...
				ServiceBusinessObjects.AzManStoreAttribute storeAttribute = this.FindAttribute(this._store.Attributes, frm.key);
		...
		private void CommitChanges()
		{
			try
			{
				...
				var _attributes = new List<ServiceBusinessObjects.AzManStoreAttribute>();
				foreach (ListViewItem _i in lsvAttributes.Items)
				{
					if (_i.Tag as ServiceBusinessObjects.AzManStoreAttribute != null)
					{
						_attributes.Add(((ServiceBusinessObjects.AzManStoreAttribute)_i.Tag).Clone());
					}
					else
					{
						var _attr = (KeyValuePair<string, string>)_i.Tag;
						_attributes.Add(new ServiceBusinessObjects.AzManStoreAttribute()
						{
							Key = _attr.Key,
							Value = _attr.Value
						});
				...
				_store.Attributes = _attributes;

				ServiceBusinessObjects.AzManStore _updated;
				#region Call WebApi
				var _h = new AzManWebApiClientHelpers.AzManStoresHelper<NetSqlAzMan.ServiceBusinessObjects.AzManStore>(_webApiUri);
				var _return = Task.Run(() => _h.PutAsync(_store.Name, _store)).Result;
				...
				//	_updated = (ServiceBusinessObjects.AzManStore)_values[0];
				//}
				#endregion
		...
		private void btnRemove_Click(object sender, EventArgs e)
		{
			if (this.lsvAttributes.SelectedItems.Count > 0)
			{
				...
				{
					if ((lvi.Tag as ServiceBusinessObjects.AzManStoreAttribute) != null)
					{
						var lviTag = (ServiceBusinessObjects.AzManStoreAttribute)lvi.Tag;
						this.attributesToRemove.Add(new KeyValuePair<string, string>(lviTag.Key, lviTag.Value));
					}
					else //if ((lvi.Tag as KeyValuePair<string,string>) != null)
					{
		...
		private void lsvAttributes_ItemCheck(object sender, ItemCheckEventArgs e)
		{
			if (this.lsvAttributes.CheckedItems.Count == 1 && e.CurrentValue == CheckState.Checked && e.NewValue == CheckState.Unchecked)
				...
				this.btnRemove.Enabled = true && this._store.IAmManager;
		}
		...
		{
			frmAttributeProperties frm = new frmAttributeProperties();
			if (this.lsvAttributes.SelectedItems[0].Tag as ServiceBusinessObjects.AzManStoreAttribute != null)
			{
				var attr = this.lsvAttributes.SelectedItems[0].Tag as ServiceBusinessObjects.AzManStoreAttribute;
				frm.key = attr.Key;
		...
		{
			this.btnEdit.Enabled = (this.lsvAttributes.SelectedItems.Count == 1) && this._store.IAmManager;
		}

		private void lsvAttributes_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
		{
			this.btnRemove.Enabled = this.lsvAttributes.SelectedItems.Count > 0 && this._store.IAmManager;
			this.btnEdit.Enabled = this.lsvAttributes.SelectedItems.Count == 1 && this._store.IAmManager;
		}
