...
namespace AzManWinUI {
	public partial class SettingsUI : Form {
		...
		private void showCurrentSettings() {
			//General
			combTreeNodeSize.SelectedValue = _settings.ManagerUI_TreeNodeSize;
			combListViewType.SelectedValue = _settings.ManagerUI_ListViewType;

			//Language
			switch (_settings.ManagerUI_Culture) {
				case "en":
				...
				default:
					MessageBox.Show(String.Format("No se puede marcar las opciones para la cultura {0}", _settings.ManagerUI_Culture));
					break;
			}

			//View
			combStructureView.SelectedValue = _settings.ManagerUI_StructureView;
			combAuthorizationsView.SelectedValue = _settings.ManagerUI_AuthorizationView;
		}
		...
		private bool saveSettings(out Exception hex) {
			...
			try {
				//General
				_settings.ManagerUI_TreeNodeSize = (TreeNodeSizeEnum)combTreeNodeSize.SelectedValue;
				_settings.ManagerUI_ListViewType = (ListViewTypeEnum)combListViewType.SelectedValue;

				//Language
				if (radbEnglish.Checked)
					_settings.ManagerUI_Culture = "en";
				else if (radbEspañol.Checked)
					_settings.ManagerUI_Culture = "es";

				//View				
				_settings.ManagerUI_StructureView = (StructureViewEnum)combStructureView.SelectedValue;
				_settings.ManagerUI_AuthorizationView = (AuthorizationViewEnum)combAuthorizationsView.SelectedValue;
	...
	public class StructureView {
		private string _language = Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2);
				...
				switch (pvenumId) {
					case StructureViewEnum.RoleTask:
						if (_language.Equals("es"))
							striName = "Rol-Tarea-Operación";
						else
							striName = "Role-Task-Operation";
						break;

					case StructureViewEnum.Role:
						if (_language.Equals("es"))
							striName = "Rol-Operación";
						else
							striName = "Role-Operation";
						break;
