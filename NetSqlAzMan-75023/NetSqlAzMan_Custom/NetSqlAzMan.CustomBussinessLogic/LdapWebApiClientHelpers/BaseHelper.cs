...
namespace NetSqlAzMan.CustomBussinessLogic.LdapWebApiClientHelpers
{
	internal class BaseHelper<BSO>
	{
		...
		private string getFullUri(string requestUri) {
			return WebApiUri + "/" + requestUri;
		}

		private WebApiStandard.WebApiCommon.WebApiRequestException getHttpWebApiRequestException(string requestUri, HttpResponseMessage responseMessage) {
			var _statusCode = responseMessage.StatusCode;

			//Validar StatusCode entre 200 y 299
			if (_statusCode >= System.Net.HttpStatusCode.OK && _statusCode < System.Net.HttpStatusCode.MultipleChoices)
				throw new InvalidOperationException("El código de estado de la respuesta es satisfactorio (200-299). No se puede generar el objeto HttpWebApiRequestException.");

			var _statusInfo = string.Format("{0}: {1}", Convert.ToInt32(_statusCode).ToString(), responseMessage.ReasonPhrase);

			string _mediaType;
			if (responseMessage.Content.Headers.ContentLength.Value.Equals(0))
				_mediaType = Global.MimeType_NoContent;
			else
				_mediaType = responseMessage.Content.Headers.ContentType.MediaType;

			switch (_mediaType) {
				case Global.MimeType_NoContent:
					var _emptyContent = new WebApiStandard.WebApiCommon.EmptyContentErrorResponse() {
					...
					return new WebApiStandard.WebApiCommon.WebApiRequestException(string.Format("Error al realizar la solicitud web {0}", requestUri), getFullUri(requestUri), responseMessage.StatusCode, WebApiStandard.WebApiCommon.ContenType.NoContent, responseMessage.ReasonPhrase, _emptyContent);

				case Global.MimeType_AppJson:
					...
					//return new HttpWebApiRequestException(string.Format("Error al realizar la solicitud web {0}", requestUri), getFullUri(requestUri), responseMessage.StatusCode, responseMessage.ReasonPhrase, _deserializedContent);					

					var _deserializedContent = JsonConvert.DeserializeObject<WebApiStandard.WebApiCommon.JsonContentErrorResponse>(_jsonContent);
					return new WebApiStandard.WebApiCommon.WebApiRequestException(string.Format("Error al realizar la solicitud web {0}", requestUri), getFullUri(requestUri), responseMessage.StatusCode, WebApiStandard.WebApiCommon.ContenType.AppJson, responseMessage.ReasonPhrase, _deserializedContent);

				case Global.MimeType_TextHtml:
					var _viewModelForHtml = new WebApiStandard.WebApiCommon.TextHtmlContentErrorResponse(Task.Run(() => responseMessage.Content.ReadAsStringAsync()).Result);

					return new WebApiStandard.WebApiCommon.WebApiRequestException(string.Format("Error al realizar la solicitud web {0}", requestUri), getFullUri(requestUri), responseMessage.StatusCode, WebApiStandard.WebApiCommon.ContenType.TextHtml, responseMessage.ReasonPhrase, _viewModelForHtml);
		...
		protected Dictionary<string, IEnumerable<object>> GetStoredResponseError(string requestUri, HttpResponseMessage respMsg) {
			var _exception = this.getHttpWebApiRequestException(requestUri, respMsg);
		...
		public void ThrowWebApiRequestException(Dictionary<string, IEnumerable<object>> responseContent) {
			...
			var _exception = (WebApiStandard.WebApiCommon.WebApiRequestException)_values[0];

			throw _exception;
			//var _requestUri = _values[0].ToString();
			//var _respMsg = (HttpResponseMessage)_values[1];
			//throw getHttpWebApiRequestException(_requestUri, _respMsg);
		}
