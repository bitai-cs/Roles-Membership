...
USE [NetSqlAzMan.62825]
GO

/****** Object:  Schema [Banking]    Script Date: 03/26/2011 16:20:48 ******/
IF NOT EXISTS ( SELECT  *
                FROM    sys.schemas
                WHERE   name = N'Basgosoft' ) 
   EXEC sys.sp_executesql N'CREATE SCHEMA [Basgosoft] AUTHORIZATION [dbo]'
GO
...
GO
CREATE PROCEDURE [dbo].[netsqlazman_ExecuteLDAPQuery]
(
  @LDAPPATH NVARCHAR(4000),
  @LDAPQUERY NVARCHAR(4000),
  @members_cur CURSOR VARYING OUTPUT
)
AS -- REMEMBER !!!
...
IF @LDAPQUERY IS NULL
   OR RTRIM(LTRIM(@LDAPQUERY)) = ''
   OR @LDAPPATH IS NULL
   OR RTRIM(LTRIM(@LDAPPATH)) = '' 
   BEGIN
...
ELSE 
   ...
   END
SET @QUERY = CHAR(39) + '<' + 'LDAP://' + @LDAPROOTDSEPART
   + '>;(&(!(objectClass=computer))(&(|(objectClass=user)(objectClass=group)))' + @LDAPQUERYPART + ');objectSid;subtree'
   + CHAR(39) 
...
(
  [BizRuleId] [int] IDENTITY(1, 1)
                    NOT NULL,
  [BizRuleSource] [text] NOT NULL,
  [BizRuleLanguage] [tinyint] NOT NULL,
  [CompiledAssembly] [image] NOT NULL,
  CONSTRAINT [PK_BizRules] PRIMARY KEY CLUSTERED ( [BizRuleId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY] TEXTIMAGE_ON [PRIMARY]
GO
/****** Object:  Table [Basgosoft].[User]    Script Date: 06/11/2009 17:45:32 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
(
  ...
  [UserSatusId] [smallint] NOT NULL, /*0=Inactivo 1=Activo*/
  CONSTRAINT [PK_Membership] PRIMARY KEY CLUSTERED ( [UserID] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
(
  ...
  CONSTRAINT [PK_ItemsHierarchy] PRIMARY KEY CLUSTERED ( [ItemId] ASC, [MemberOfItemId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ItemsHierarchy] ON [dbo].[netsqlazman_ItemsHierarchyTable] ( [ItemId] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ItemsHierarchy_1] ON [dbo].[netsqlazman_ItemsHierarchyTable] ( [MemberOfItemId] ASC )
ON [PRIMARY]
GO
...
(
  ...
  [SqlIdentity] [nvarchar](128) NULL
                                CONSTRAINT [DF_Log_SqlIdentity] DEFAULT ( SUSER_SNAME() ),
  ...
  CONSTRAINT [PK_Log] PRIMARY KEY NONCLUSTERED ( [LogId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
SET ANSI_PADDING ON
GO
CREATE CLUSTERED INDEX [IX_Log_2] ON [dbo].[netsqlazman_LogTable] ( [LogDateTime] DESC, [InstanceGuid] ASC, [OperationCounter] DESC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_Log] ON [dbo].[netsqlazman_LogTable] ( [WindowsIdentity] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_Log_1] ON [dbo].[netsqlazman_LogTable] ( [SqlIdentity] ASC )
ON [PRIMARY]
GO
...
(
  ...
  CONSTRAINT [PK_Settings] PRIMARY KEY CLUSTERED ( [SettingName] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE FUNCTION [dbo].[netsqlazman_MergeAuthorizations]
(
  @AUTH1 TINYINT,
  @AUTH2 TINYINT
)
...
-- 0 Neutral 1 Allow 2 Deny 3 AllowWithDelegation
   ...
   ELSE 
      ...
      ELSE 
         BEGIN
            IF @AUTH1 = 2 
               SET @AUTH1 = 4 -- DENY WINS
            ELSE 
               IF @AUTH2 = 2 
                  SET @AUTH2 = 4 -- DENY WINS
            IF @AUTH1 >= @AUTH2 
               SET @RESULT = @AUTH1
            ELSE 
               IF @AUTH1 < @AUTH2 
...
BEGIN 
   ...
   IF IS_MEMBER('db_owner') = 1
      OR IS_MEMBER('NetSqlAzMan_Administrators') = 1 
      SET @result = 1
   ELSE 
      SET @result = 0
   RETURN @result
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[netsqlazman_helplogins]
(
...
IF @rolename = 'NetSqlAzMan_Managers' 
   BEGIN
      INSERT   INTO #temptable
               EXEC sp_helprolemember 'NetSqlAzMan_Managers'
   END
...
IF @rolename = 'NetSqlAzMan_Users' 
   BEGIN
      INSERT   INTO #temptable
               EXEC sp_helprolemember 'NetSqlAzMan_Managers'
      INSERT   INTO #temptable
               EXEC sp_helprolemember 'NetSqlAzMan_Users'
   END
...
IF @rolename = 'NetSqlAzMan_Readers' 
   BEGIN
      ...
      INSERT   INTO #temptable
               EXEC sp_helprolemember 'NetSqlAzMan_Users'
      INSERT   INTO #temptable
               EXEC sp_helprolemember 'NetSqlAzMan_Readers'
   END

SELECT DISTINCT
         SUSER_SNAME(MemberSid) SqlUserOrRole,
         CASE MemberSid
           WHEN NULL THEN 1
           ELSE 0
         END AS IsSqlRole
FROM     #temptable
WHERE    MemberName NOT IN ( 'NetSqlAzMan_Administrators', 'NetSqlAzMan_Managers', 'NetSqlAzMan_Users',
                             'NetSqlAzMan_Readers' )
...
(
  ...
  CONSTRAINT [PK_Stores] PRIMARY KEY CLUSTERED ( [StoreId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [Stores_Name_Unique_Index] ON [dbo].[netsqlazman_StoresTable] ( [Name] ASC )
ON [PRIMARY]
GO
...
(
  [AuthorizationAttributeId] [int] IDENTITY(1, 1)
                                   NOT NULL,
  [AuthorizationId] [int] NOT NULL,
  [AttributeKey] [nvarchar](255) NOT NULL,
  [AttributeValue] [nvarchar](4000) NOT NULL,
  CONSTRAINT [PK_AuthorizationAttributes] PRIMARY KEY CLUSTERED ( [AuthorizationAttributeId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [AuthorizationAttributes_AuhorizationId_AttributeKey_Unique_Index] ON [dbo].[netsqlazman_AuthorizationAttributesTable] ( [AuthorizationId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_AuthorizationAttributes] ON [dbo].[netsqlazman_AuthorizationAttributesTable] ( [AuthorizationId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
...
(
  [ApplicationGroupMemberId] [int] IDENTITY(1, 1)
                                   NOT NULL,
  [ApplicationGroupId] [int] NOT NULL,
  [objectSid] [varbinary](85) NOT NULL,
  [WhereDefined] [tinyint] NOT NULL,
  [IsMember] [bit] NOT NULL,
  CONSTRAINT [PK_GroupMembers] PRIMARY KEY CLUSTERED ( [ApplicationGroupMemberId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
SET ANSI_PADDING ON
GO
CREATE UNIQUE NONCLUSTERED INDEX [ApplicationGroupMembers_ApplicationGroupId_ObjectSid_IsMember_Unique_Index] ON [dbo].[netsqlazman_ApplicationGroupMembersTable] ( [ApplicationGroupId] ASC, [objectSid] ASC, [IsMember] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ApplicationGroupMembers] ON [dbo].[netsqlazman_ApplicationGroupMembersTable] ( [ApplicationGroupId] ASC, [objectSid] ASC )
ON [PRIMARY]
GO
...
(
  ...
  [BizRuleId] [int] NULL,
  CONSTRAINT [PK_Items] PRIMARY KEY CLUSTERED ( [ItemId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [Items_ApplicationId_Name_Unique_Index] ON [dbo].[netsqlazman_ItemsTable] ( [Name] ASC, [ApplicationId] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_Items] ON [dbo].[netsqlazman_ItemsTable] ( [ApplicationId] ASC, [Name] ASC )
ON [PRIMARY]
GO
...
(
  [ApplicationGroupId] [int] IDENTITY(1, 1)
                             NOT NULL,
  [ApplicationId] [int] NOT NULL,
  [objectSid] [varbinary](85) NOT NULL,
  [Name] [nvarchar](255) NOT NULL,
  ...
  CONSTRAINT [PK_Groups] PRIMARY KEY CLUSTERED ( [ApplicationGroupId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
SET ANSI_PADDING ON
GO
CREATE UNIQUE NONCLUSTERED INDEX [ApplicationGroups_ApplicationId_Name_Unique_Index] ON [dbo].[netsqlazman_ApplicationGroupsTable] ( [ApplicationId] ASC, [Name] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ApplicationGroups] ON [dbo].[netsqlazman_ApplicationGroupsTable] ( [ApplicationId] ASC, [Name] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ApplicationGroups_1] ON [dbo].[netsqlazman_ApplicationGroupsTable] ( [objectSid] ASC )
ON [PRIMARY]
GO
...
(
  [ApplicationAttributeId] [int] IDENTITY(1, 1)
                                 NOT NULL,
  [ApplicationId] [int] NOT NULL,
  [AttributeKey] [nvarchar](255) NOT NULL,
  [AttributeValue] [nvarchar](4000) NOT NULL,
  CONSTRAINT [PK_ApplicationAttributes] PRIMARY KEY CLUSTERED ( [ApplicationAttributeId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [ApplicationAttributes_AuhorizationId_AttributeKey_Unique_Index] ON [dbo].[netsqlazman_ApplicationAttributesTable] ( [ApplicationId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ApplicationAttributes] ON [dbo].[netsqlazman_ApplicationAttributesTable] ( [ApplicationId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
...
(
  [ApplicationPermissionId] [int] IDENTITY(1, 1)
                                  NOT NULL,
  [ApplicationId] [int] NOT NULL,
  [SqlUserOrRole] [nvarchar](128) NOT NULL,
  [IsSqlRole] [bit] NOT NULL,
  [NetSqlAzManFixedServerRole] [tinyint] NOT NULL,
  CONSTRAINT [PK_ApplicationPermissions] PRIMARY KEY CLUSTERED ( [ApplicationPermissionId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ApplicationPermissions] ON [dbo].[netsqlazman_ApplicationPermissionsTable] ( [ApplicationId] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ApplicationPermissions_1] ON [dbo].[netsqlazman_ApplicationPermissionsTable] ( [ApplicationId] ASC, [SqlUserOrRole] ASC, [NetSqlAzManFixedServerRole] ASC )
ON [PRIMARY]
GO
...
(
  [StoreGroupMemberId] [int] IDENTITY(1, 1)
                             NOT NULL,
  [StoreGroupId] [int] NOT NULL,
  [objectSid] [varbinary](85) NOT NULL,
  [WhereDefined] [tinyint] NOT NULL,
  [IsMember] [bit] NOT NULL,
  CONSTRAINT [PK_StoreGroupMembers] PRIMARY KEY CLUSTERED ( [StoreGroupMemberId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
SET ANSI_PADDING ON
GO
CREATE NONCLUSTERED INDEX [IX_StoreGroupMembers] ON [dbo].[netsqlazman_StoreGroupMembersTable] ( [StoreGroupId] ASC, [objectSid] ASC )
ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [StoreGroupMembers_StoreGroupId_ObjectSid_IsMember_Unique_Index] ON [dbo].[netsqlazman_StoreGroupMembersTable] ( [StoreGroupId] ASC, [objectSid] ASC, [IsMember] ASC )
ON [PRIMARY]
GO
...
(
  ...
  [Name] [nvarchar](255) NOT NULL,
  [Description] [nvarchar](1024) NOT NULL,
  CONSTRAINT [PK_Applications] PRIMARY KEY CLUSTERED ( [ApplicationId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [Applications_StoreId_Name_Unique_Index] ON [dbo].[netsqlazman_ApplicationsTable] ( [Name] ASC, [StoreId] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_Applications] ON [dbo].[netsqlazman_ApplicationsTable] ( [ApplicationId] ASC, [Name] ASC )
ON [PRIMARY]
GO
...
(
  [StoreAttributeId] [int] IDENTITY(1, 1)
                           NOT NULL,
  [StoreId] [int] NOT NULL,
  [AttributeKey] [nvarchar](255) NOT NULL,
  [AttributeValue] [nvarchar](4000) NOT NULL,
  CONSTRAINT [PK_StoreAttributes] PRIMARY KEY CLUSTERED ( [StoreAttributeId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_StoreAttributes] ON [dbo].[netsqlazman_StoreAttributesTable] ( [StoreId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [StoreAttributes_AuhorizationId_AttributeKey_Unique_Index] ON [dbo].[netsqlazman_StoreAttributesTable] ( [StoreId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
...
(
  [StorePermissionId] [int] IDENTITY(1, 1)
                            NOT NULL,
  [StoreId] [int] NOT NULL,
  [SqlUserOrRole] [nvarchar](128) NOT NULL,
  [IsSqlRole] [bit] NOT NULL,
  [NetSqlAzManFixedServerRole] [tinyint] NOT NULL,
  CONSTRAINT [PK_StorePermissions] PRIMARY KEY CLUSTERED ( [StorePermissionId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_StorePermissions] ON [dbo].[netsqlazman_StorePermissionsTable] ( [StoreId] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_StorePermissions_1] ON [dbo].[netsqlazman_StorePermissionsTable] ( [StoreId] ASC, [SqlUserOrRole] ASC, [NetSqlAzManFixedServerRole] ASC )
ON [PRIMARY]
GO
...
(
  ...
  [LDapQuery] [nvarchar](4000) NULL,
  [GroupType] [tinyint] NOT NULL,
  CONSTRAINT [PK_StoreGroups] PRIMARY KEY CLUSTERED ( [StoreGroupId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
SET ANSI_PADDING ON
GO
CREATE NONCLUSTERED INDEX [IX_StoreGroups] ON [dbo].[netsqlazman_StoreGroupsTable] ( [StoreId] ASC, [objectSid] ASC )
ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [StoreGroups_StoreId_Name_Unique_Index] ON [dbo].[netsqlazman_StoreGroupsTable] ( [StoreId] ASC, [Name] ASC )
ON [PRIMARY]
GO
...
(
  [AuthorizationId] [int] IDENTITY(1, 1)
                          NOT NULL,
  [ItemId] [int] NOT NULL,
  ...
  CONSTRAINT [PK_Authorizations] PRIMARY KEY CLUSTERED ( [AuthorizationId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
SET ANSI_PADDING ON
GO
CREATE NONCLUSTERED INDEX [IX_Authorizations] ON [dbo].[netsqlazman_AuthorizationsTable] ( [ItemId] ASC, [objectSid] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_Authorizations_1] ON [dbo].[netsqlazman_AuthorizationsTable] ( [ItemId] ASC, [objectSid] ASC, [objectSidWhereDefined] ASC, [AuthorizationType] ASC, [ValidFrom] ASC, [ValidTo] ASC )
ON [PRIMARY]
GO
...
(
  [ItemAttributeId] [int] IDENTITY(1, 1)
                          NOT NULL,
  [ItemId] [int] NOT NULL,
  [AttributeKey] [nvarchar](255) NOT NULL,
  [AttributeValue] [nvarchar](4000) NOT NULL,
  CONSTRAINT [PK_ItemAttributes] PRIMARY KEY CLUSTERED ( [ItemAttributeId] ASC ) ON [PRIMARY]
)
   ON
[PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [ItemAttributes_AuhorizationId_AttributeKey_Unique_Index] ON [dbo].[netsqlazman_ItemAttributesTable] ( [ItemId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_ItemAttributes] ON [dbo].[netsqlazman_ItemAttributesTable] ( [ItemId] ASC, [AttributeKey] ASC )
ON [PRIMARY]
GO
...
DECLARE applicationgroups_cur CURSOR FAST_FORWARD
   FOR SELECT  objectSid
       FROM    deleted
...
WHILE @@FETCH_STATUS = 0
   BEGIN
      ...
      WHERE    objectSid = @DELETEDOBJECTSID
               AND WhereDefined = 1
      DELETE   FROM dbo.[netsqlazman_AuthorizationsTable]
      WHERE    objectSid = @DELETEDOBJECTSID
               AND objectSidWhereDefined = 1
      FETCH NEXT FROM applicationgroups_cur INTO @DELETEDOBJECTSID
   END
...
AS
DECLARE @DELETEDOBJECTSID INT
DECLARE storegroups_cur CURSOR FAST_FORWARD
   FOR SELECT  objectSid
       FROM    deleted
...
*/
CREATE FUNCTION [dbo].[netsqlazman_CheckApplicationPermissions]
(
  @ApplicationId INT,
  @ROLEID TINYINT
)
...
BEGIN
   ...
   ELSE 
      BEGIN
         IF EXISTS ( SELECT   dbo.[netsqlazman_ApplicationPermissionsTable].ApplicationId
                     FROM     dbo.[netsqlazman_ApplicationsTable]
                              INNER JOIN dbo.[netsqlazman_StoresTable] ON dbo.[netsqlazman_ApplicationsTable].StoreId = dbo.[netsqlazman_StoresTable].StoreId
                              LEFT OUTER JOIN dbo.[netsqlazman_StorePermissionsTable] ON dbo.[netsqlazman_StoresTable].StoreId = dbo.[netsqlazman_StorePermissionsTable].StoreId
                              LEFT OUTER JOIN dbo.[netsqlazman_ApplicationPermissionsTable] ON dbo.[netsqlazman_ApplicationsTable].ApplicationId = dbo.[netsqlazman_ApplicationPermissionsTable].ApplicationId
                     WHERE    IS_MEMBER('db_owner') = 1
                              OR IS_MEMBER('NetSqlAzMan_Administrators') = 1
                              OR ( @ROLEID = 0
                                   AND IS_MEMBER('NetSqlAzMan_Readers') = 1
                                   OR @ROLEID = 1
                                   AND IS_MEMBER('NetSqlAzMan_Users') = 1
                                   OR @ROLEID = 2
                                   AND IS_MEMBER('NetSqlAzMan_Managers') = 1 )
                              AND ( ( dbo.[netsqlazman_ApplicationPermissionsTable].ApplicationId = @ApplicationId
                                      AND dbo.[netsqlazman_ApplicationPermissionsTable].NetSqlAzManFixedServerRole >= @ROLEID
                                      AND ( SUSER_SNAME(SUSER_SID()) = [netsqlazman_ApplicationPermissionsTable].SqlUserOrRole
                                            AND [netsqlazman_ApplicationPermissionsTable].IsSqlRole = 0
                                            OR IS_MEMBER([netsqlazman_ApplicationPermissionsTable].SqlUserOrRole) = 1
                                            AND [netsqlazman_ApplicationPermissionsTable].IsSqlRole = 1 ) )
                                    OR dbo.[netsqlazman_ApplicationsTable].ApplicationId = @ApplicationId
                                    AND ( dbo.[netsqlazman_StorePermissionsTable].StoreId = dbo.[netsqlazman_ApplicationsTable].StoreId
                                          AND dbo.[netsqlazman_StorePermissionsTable].NetSqlAzManFixedServerRole >= @ROLEID
                                          AND ( SUSER_SNAME(SUSER_SID()) = [netsqlazman_StorePermissionsTable].SqlUserOrRole
                                                AND [netsqlazman_StorePermissionsTable].IsSqlRole = 0
                                                OR IS_MEMBER([netsqlazman_StorePermissionsTable].SqlUserOrRole) = 1
                                                AND [netsqlazman_StorePermissionsTable].IsSqlRole = 1 ) ) ) ) 
            SET @RESULT = 1
         ELSE 
            SET @RESULT = 0
      END
...
/* 
   @ROLEID = { 0 READERS, 1 USERS, 2 MANAGERS}
*/
CREATE FUNCTION [dbo].[netsqlazman_CheckStorePermissions]
(
  @STOREID INT,
  @ROLEID TINYINT
)
...
BEGIN
   ...
   ELSE 
      BEGIN
         IF EXISTS ( SELECT   dbo.[netsqlazman_StorePermissionsTable].StoreId
                     FROM     dbo.[netsqlazman_ApplicationsTable]
                              RIGHT OUTER JOIN dbo.[netsqlazman_StoresTable] ON dbo.[netsqlazman_ApplicationsTable].StoreId = dbo.[netsqlazman_StoresTable].StoreId
                              LEFT OUTER JOIN dbo.[netsqlazman_StorePermissionsTable] ON dbo.[netsqlazman_StoresTable].StoreId = dbo.[netsqlazman_StorePermissionsTable].StoreId
                              LEFT OUTER JOIN dbo.[netsqlazman_ApplicationPermissionsTable] ON dbo.[netsqlazman_ApplicationsTable].ApplicationId = dbo.[netsqlazman_ApplicationPermissionsTable].ApplicationId
                     WHERE    IS_MEMBER('db_owner') = 1
                              OR IS_MEMBER('NetSqlAzMan_Administrators') = 1
                              OR ( @ROLEID = 0
                                   ...
                                   OR @ROLEID = 2
                                   AND IS_MEMBER('NetSqlAzMan_Managers') = 1 )
                              AND ( ( dbo.[netsqlazman_StorePermissionsTable].StoreId = @STOREID
                                      AND dbo.[netsqlazman_StorePermissionsTable].NetSqlAzManFixedServerRole >= @ROLEID
                                      AND ( SUSER_SNAME(SUSER_SID()) = [netsqlazman_StorePermissionsTable].SqlUserOrRole
                                            AND [netsqlazman_StorePermissionsTable].IsSqlRole = 0
                                            OR IS_MEMBER([netsqlazman_StorePermissionsTable].SqlUserOrRole) = 1
                                            AND [netsqlazman_StorePermissionsTable].IsSqlRole = 1 ) )
                                    OR ( @ROLEID = 0
                                         AND dbo.[netsqlazman_StoresTable].StoreId = @STOREID
                                         AND dbo.[netsqlazman_ApplicationPermissionsTable].NetSqlAzManFixedServerRole >= @ROLEID
                                         AND ( SUSER_SNAME(SUSER_SID()) = [netsqlazman_ApplicationPermissionsTable].SqlUserOrRole
                                               AND [netsqlazman_ApplicationPermissionsTable].IsSqlRole = 0
                                               OR IS_MEMBER([netsqlazman_ApplicationPermissionsTable].SqlUserOrRole) = 1
...
AS 
INSERT   INTO [dbo].[netsqlazman_BizRulesTable]
   (
     [BizRuleSource],
     [BizRuleLanguage],
     [CompiledAssembly]
...
DECLARE items_cur CURSOR FAST_FORWARD
   FOR SELECT  ItemId,
               BizRuleId
       FROM    deleted
OPEN items_cur
...
WHILE @@FETCH_STATUS = 0
   BEGIN
      DELETE   FROM dbo.[netsqlazman_ItemsHierarchyTable]
      WHERE    ItemId = @DELETEDITEMID
               OR MemberOfItemId = @DELETEDITEMID
      IF @BIZRULEID IS NOT NULL 
         DELETE   FROM dbo.[netsqlazman_BizRulesTable]
         WHERE    BizRuleId = @BIZRULEID
      FETCH NEXT FROM items_cur INTO @DELETEDITEMID, @BIZRULEID
   END
CLOSE items_cur
...
*/
CREATE FUNCTION [dbo].[netsqlazman_GetDBUsers]
(
...
RETURN
   SELECT TOP 100 PERCENT
            CONVERT(VARBINARY(85), UserID) AS DBUserSid,
            UserName AS DBUserName,
            FullName,
            OtherFields
   FROM     Basgosoft.Membership
...
DECLARE itemhierarchy_cur CURSOR FAST_FORWARD
   FOR SELECT  ItemId,
               MemberOfItemId
       FROM    inserted
   ...
   BEGIN
      IF UPDATE(ItemId)
         AND NOT EXISTS ( SELECT ItemId
                          FROM   dbo.[netsqlazman_ItemsTable]
                          WHERE  [netsqlazman_ItemsTable].ItemId = @INSERTEDITEMID ) 
         BEGIN
            RAISERROR ( 'ItemId NOT FOUND into dbo.ItemsTable', 16, 1 )
            ROLLBACK TRANSACTION
         END
	
      IF UPDATE(MemberOfItemId)
         AND NOT EXISTS ( SELECT ItemId
                          FROM   dbo.[netsqlazman_ItemsTable]
                          WHERE  [netsqlazman_ItemsTable].ItemId = @INSERTEDMEMBEROFITEMID ) 
         BEGIN
...
AS 
INSERT   INTO [dbo].[netsqlazman_StoresTable] ( [Name], [Description] )
...
AS 
IF EXISTS ( SELECT   StoreId
            FROM     dbo.[netsqlazman_StoresTable]
            WHERE    StoreId = @StoreId )
   AND ( dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1
         AND @NetSqlAzManFixedServerRole BETWEEN 0 AND 1
         OR ( IS_MEMBER('db_owner') = 1
              OR IS_MEMBER('NetSqlAzMan_Administrators') = 1 )
         AND @NetSqlAzManFixedServerRole = 2 ) 
   BEGIN
	 ...
	 -- CHECK MEMBER NAME (ATTEMPT ADDING IMPLICIT ROW FOR NT NAME) --
      ...
      SELECT   @MEMBERUID = uid,
               @IsSqlRoleInt = issqlrole,
               @IsNtGroupInt = isntgroup
      FROM     sysusers
      WHERE    sid = SUSER_SID(@SqlUserOrRole)
               AND isaliased = 0
      IF @IsSqlRoleInt = 1
         OR @IsNtGroupInt = 1 
         SET @IsSqlRole = 1
      ...
      IF @MEMBERUID IS NULL 
         ...
         END
      IF EXISTS ( SELECT   *
                  FROM     dbo.[netsqlazman_StorePermissionsTable]
                  WHERE    StoreId = @StoreId
                           AND SqlUserOrRole = @SqlUserOrRole
                           AND NetSqlAzManFixedServerRole = @NetSqlAzManFixedServerRole ) 
         BEGIN
      ...
      ELSE 
         BEGIN
            INSERT   INTO dbo.[netsqlazman_StorePermissionsTable]
               (
                 StoreId,
                 SqlUserOrRole,
                 IsSqlRole,
                 NetSqlAzManFixedServerRole
               )
            VALUES   (
                       @StoreId,
                       @SqlUserOrRole,
   ...
   BEGIN
      IF EXISTS ( SELECT   *
                  ...
                  WHERE    StoreId = @StoreId
                           AND SqlUserOrRole = @SqlUserOrRole
                           AND NetSqlAzManFixedServerRole = @NetSqlAzManFixedServerRole ) 
         DELETE   FROM dbo.[netsqlazman_StorePermissionsTable]
         WHERE    StoreId = @StoreId
                  AND SqlUserOrRole = @SqlUserOrRole
                  AND NetSqlAzManFixedServerRole = @NetSqlAzManFixedServerRole
      ELSE 
...
AS 
IF EXISTS ( SELECT   BizRuleId
            FROM     dbo.[netsqlazman_BizRulesTable]
            WHERE    BizRuleId = @BizRuleId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_BizRulesTable]
   WHERE    [BizRuleId] = @BizRuleId
ELSE 
...
IF dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_StoreGroupsTable]
   WHERE    [StoreGroupId] = @Original_StoreGroupId
            AND [StoreId] = @StoreId
ELSE 
...
IF dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_StoreGroupsTable]
         (
           [StoreId],
           [objectSid],
           [Name],
...
AS 
IF dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_StoreGroupsTable]
   SET      [objectSid] = @objectSid,
            [Name] = @Name,
...
RETURNS TABLE
   AS
RETURN
   SELECT   dbo.[netsqlazman_StoresTable].*
   FROM     dbo.[netsqlazman_StoresTable]
...
RETURN
   ...
   FROM     dbo.[netsqlazman_ApplicationGroupsTable]
            INNER JOIN dbo.[netsqlazman_Applications]() Applications ON dbo.[netsqlazman_ApplicationGroupsTable].ApplicationId = Applications.ApplicationId
GO
...
IF EXISTS ( SELECT   ApplicationId
            FROM     dbo.[netsqlazman_Applications]()
            WHERE    ApplicationId = @ApplicationId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ApplicationGroupsTable]
         (
           [ApplicationId],
           [objectSid],
           [Name],
...
IF EXISTS ( SELECT   ApplicationId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   BEGIN
      INSERT   INTO dbo.[netsqlazman_ApplicationPermissionsTable]
         (
           ApplicationId,
           SqlUserOrRole,
           IsSqlRole,
...
IF EXISTS ( SELECT   ApplicationId
   ...
   AND ( dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1
         AND @NetSqlAzManFixedServerRole BETWEEN 0 AND 1
         OR dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1
         AND @NetSqlAzManFixedServerRole = 2 ) 
   BEGIN
      IF EXISTS ( SELECT   *
                  FROM     dbo.[netsqlazman_ApplicationPermissionsTable]
                  WHERE    ApplicationId = @ApplicationId
                           AND SqlUserOrRole = @SqlUserOrRole
                           AND NetSqlAzManFixedServerRole = @NetSqlAzManFixedServerRole ) 
         DELETE   FROM dbo.[netsqlazman_ApplicationPermissionsTable]
         WHERE    ApplicationId = @ApplicationId
                  AND SqlUserOrRole = @SqlUserOrRole
                  AND NetSqlAzManFixedServerRole = @NetSqlAzManFixedServerRole
      ELSE 
...
IF EXISTS ( SELECT   ApplicationId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   DELETE   FROM dbo.[netsqlazman_ApplicationPermissionsTable]
   WHERE    ApplicationPermissionId = @ApplicationPermissionId
...
RETURN
   ...
   FROM     dbo.[netsqlazman_ApplicationPermissionsTable]
            INNER JOIN dbo.[netsqlazman_Applications]() Applications ON dbo.[netsqlazman_ApplicationPermissionsTable].ApplicationId = Applications.ApplicationId
GO
...
IF EXISTS ( SELECT   ApplicationId
            ...
            WHERE    ApplicationId = @ApplicationId )
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_ApplicationsTable]
   WHERE    [ApplicationId] = @ApplicationId
            AND [StoreId] = @StoreId
ELSE 
   RAISERROR ( 'Store permission denied', 16, 1 )
...
AS 
IF EXISTS ( SELECT   StoreId
            FROM     dbo.[netsqlazman_Stores]()
            WHERE    StoreId = @StoreId )
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ApplicationsTable]
         (
           [StoreId],
           [Name],
           [Description]
...
IF EXISTS ( SELECT   ApplicationId
            FROM     dbo.[netsqlazman_Applications]()
            WHERE    ApplicationId = @Original_ApplicationId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@Original_ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_ApplicationsTable]
   SET      [Name] = @Name,
            [Description] = @Description
   WHERE    [ApplicationId] = @Original_ApplicationId
ELSE 
...
IF EXISTS ( SELECT   StoreId
   ...
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_StoreAttributesTable]
         (
           [StoreId],
           [AttributeKey],
           [AttributeValue]
...
RETURN
   ...
   FROM     dbo.[netsqlazman_StoreAttributesTable]
            INNER JOIN dbo.[netsqlazman_Stores]() Stores ON dbo.[netsqlazman_StoreAttributesTable].StoreId = Stores.StoreId
GO
...
IF EXISTS ( SELECT   StoreId
   ...
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   DELETE   FROM dbo.[netsqlazman_StorePermissionsTable]
   WHERE    StorePermissionId = @StorePermissionId
            AND StoreId = @StoreId
ELSE 
   RAISERROR ( 'Store permission denied.', 16, 1 )
GO
...
RETURN
   ...
   FROM     dbo.[netsqlazman_StorePermissionsTable]
            INNER JOIN dbo.[netsqlazman_Stores]() Stores ON dbo.[netsqlazman_StorePermissionsTable].StoreId = Stores.StoreId
GO
...
RETURN
   ...
   FROM     dbo.[netsqlazman_ApplicationAttributesTable]
            INNER JOIN dbo.[netsqlazman_Applications]() Applications ON dbo.[netsqlazman_ApplicationAttributesTable].ApplicationId = Applications.ApplicationId
GO
...
IF EXISTS ( SELECT   ApplicationId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ApplicationAttributesTable]
         (
           [ApplicationId],
           [AttributeKey],
           [AttributeValue]
...
RETURN
   ...
   FROM     dbo.[netsqlazman_Stores]() Stores
            INNER JOIN dbo.[netsqlazman_StoreGroupsTable] ON Stores.StoreId = dbo.[netsqlazman_StoreGroupsTable].StoreId
GO
...
IF EXISTS ( SELECT   ApplicationId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ItemsTable]
         (
           [ApplicationId],
           [Name],
           [Description],
...
IF EXISTS ( SELECT   StoreId
            FROM     dbo.[netsqlazman_Stores]()
            WHERE    StoreId = @Original_StoreId )
   AND dbo.[netsqlazman_CheckStorePermissions](@Original_StoreId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_StoresTable]
   SET      [Name] = @Name,
            [Description] = @Description
   WHERE    [StoreId] = @Original_StoreId
ELSE 
...
AS SELECT   [netsqlazman_Stores].StoreId,
            [netsqlazman_Stores].Name AS StoreName,
            [netsqlazman_Stores].Description AS StoreDescription,
            [netsqlazman_Applications].ApplicationId,
            [netsqlazman_Applications].Name AS ApplicationName,
            [netsqlazman_Applications].Description AS ApplicationDescription
   FROM     dbo.[netsqlazman_Applications]() [netsqlazman_Applications]
            INNER JOIN dbo.[netsqlazman_Stores]() [netsqlazman_Stores] ON [netsqlazman_Applications].StoreId = [netsqlazman_Stores].StoreId
GO
...
RETURN
   ...
   FROM     dbo.[netsqlazman_ItemsTable]
            INNER JOIN dbo.[netsqlazman_Applications]() Applications ON dbo.[netsqlazman_ItemsTable].ApplicationId = Applications.ApplicationId
GO
...
CREATE PROCEDURE [dbo].[netsqlazman_CreateDelegate]
(
  @ITEMID INT,
  @OWNERSID VARBINARY(85),
  @OWNERSIDWHEREDEFINED TINYINT,
  @DELEGATEDUSERSID VARBINARY(85),
  @SIDWHEREDEFINED TINYINT,
...
FROM     dbo.[netsqlazman_Items]()
WHERE    ItemId = @ItemId
IF @ApplicationId IS NOT NULL
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 1) = 1 
   BEGIN
      INSERT   INTO dbo.[netsqlazman_AuthorizationsTable]
         (
           ItemId,
           ownerSid,
...
RETURN
   ...
   FROM     dbo.[netsqlazman_AuthorizationsTable]
            INNER JOIN dbo.[netsqlazman_Items]() Items ON dbo.[netsqlazman_AuthorizationsTable].ItemId = Items.ItemId
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[netsqlazman_GetStoreGroupSidMembers]
(
  @ISMEMBER BIT,
  @GROUPOBJECTSID VARBINARY(85),
  @NETSQLAZMANMODE BIT,
  @LDAPPATH NVARCHAR(4000),
  @member_cur CURSOR VARYING OUTPUT
)
AS 
...
SELECT   @GROUPID = StoreGroupId,
         @GROUPTYPE = GroupType,
         @LDAPQUERY = LDapQuery
FROM     dbo.[netsqlazman_StoreGroups]()
WHERE    objectSid = @GROUPOBJECTSID
	...
	-- Windows SIDs
      INSERT   INTO @RESULT ( objectSid )
               ...
               WHERE    StoreGroupId = @GROUPID
                        ...
                        AND ( ( @NETSQLAZMANMODE = 0
                                AND ( WhereDefined = 2
                                      OR WhereDefined = 4 ) )
                              OR ( @NETSQLAZMANMODE = 1
                                   AND WhereDefined BETWEEN 2 AND 4 ) )
	-- Store Groups Members
      DECLARE @MemberObjectSid VARBINARY(85)
      DECLARE @MemberType BIT
      DECLARE @NotMemberType BIT
      DECLARE nested_Store_groups_cur CURSOR LOCAL FAST_FORWARD
         FOR SELECT  objectSid,
                     IsMember
             FROM    dbo.[netsqlazman_StoreGroupMembersTable]
             WHERE   StoreGroupId = @GROUPID
            ...
            ELSE 
               BEGIN
                  ...
                  ELSE 
                     SET @NotMemberType = 0
               END
            EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] @NotMemberType, @MemberObjectSid, @NETSQLAZMANMODE, @LDAPPATH,
               @sub_members_cur OUTPUT
...
AS 
IF EXISTS ( SELECT   ApplicationGroupId
            FROM     dbo.[netsqlazman_ApplicationGroups]()
            WHERE    ApplicationGroupId = @ApplicationGroupId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_ApplicationGroupsTable]
   WHERE    [ApplicationGroupId] = @ApplicationGroupId
            AND [ApplicationId] = @ApplicationId
ELSE 
   RAISERROR ( 'Application permission denied.', 16, 1 )
...
IF EXISTS ( SELECT   ApplicationGroupId
            FROM     dbo.[netsqlazman_ApplicationGroups]()
            WHERE    ApplicationGroupId = @Original_ApplicationGroupId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_ApplicationGroupsTable]
   SET      [objectSid] = @objectSid,
            [Name] = @Name,
            [Description] = @Description,
            [LDapQuery] = @LDapQuery,
...
IF EXISTS ( SELECT   ItemId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ItemAttributesTable]
         (
           [ItemId],
           [AttributeKey],
           [AttributeValue]
...
RETURN
   ...
   FROM     dbo.[netsqlazman_ItemAttributesTable]
            INNER JOIN dbo.[netsqlazman_Items]() Items ON dbo.[netsqlazman_ItemAttributesTable].ItemId = Items.ItemId
GO
...
RETURN
   ...
   FROM     dbo.[netsqlazman_StoreGroupMembersTable]
            INNER JOIN dbo.[netsqlazman_StoreGroups]() StoreGroups ON dbo.[netsqlazman_StoreGroupMembersTable].StoreGroupId = StoreGroups.StoreGroupId
GO
...
AS 
IF EXISTS ( SELECT   StoreGroupId
            FROM     dbo.[netsqlazman_StoreGroups]()
            WHERE    StoreGroupId = @StoreGroupId )
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_StoreGroupMembersTable]
         (
           [StoreGroupId],
           [objectSid],
           [WhereDefined],
...
AS 
IF EXISTS ( SELECT   StoreAttributeId
            FROM     dbo.[netsqlazman_StoreAttributes]()
            WHERE    StoreAttributeId = @Original_StoreAttributeId )
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_StoreAttributesTable]
...
IF EXISTS ( SELECT   StoreAttributeId
            FROM     dbo.[netsqlazman_StoreAttributes]()
            WHERE    StoreAttributeId = @StoreAttributeId )
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_StoreAttributesTable]
   WHERE    [StoreAttributeId] = @StoreAttributeId
            AND [StoreId] = @StoreId
ELSE 
   RAISERROR ( 'Store permission denied.', 16, 1 )
...
AS 
IF EXISTS ( SELECT   ApplicationAttributeId
            FROM     dbo.[netsqlazman_ApplicationAttributes]()
            WHERE    ApplicationAttributeId = @ApplicationAttributeId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_ApplicationAttributesTable]
   WHERE    [ApplicationAttributeId] = @ApplicationAttributeId
            AND [ApplicationId] = @ApplicationId
ELSE 
   RAISERROR ( 'Application permission denied.', 16, 1 )
...
IF EXISTS ( SELECT   ApplicationAttributeId
            FROM     dbo.[netsqlazman_ApplicationAttributes]()
            WHERE    ApplicationAttributeId = @Original_ApplicationAttributeId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_ApplicationAttributesTable]
   SET      [AttributeKey] = @AttributeKey,
            [AttributeValue] = @AttributeValue
   WHERE    [ApplicationAttributeId] = @Original_ApplicationAttributeId
            AND [ApplicationId] = @ApplicationId
...
RETURN
   ...
   FROM     dbo.[netsqlazman_BizRulesTable]
            INNER JOIN dbo.[netsqlazman_Items]() Items ON dbo.[netsqlazman_BizRulesTable].BizRuleId = Items.BizRuleId
GO
...
-- =============================================
CREATE FUNCTION [dbo].[netsqlazman_GetNameFromSid]
(
  @StoreName NVARCHAR(255),
  @ApplicationName NVARCHAR(255),
  @sid VARBINARY(85),
...
BEGIN
   ...
   IF ( @SidWhereDefined = 0 ) --Store
      ...
      END
   ELSE 
      IF ( @SidWhereDefined = 1 ) --Application 
         BEGIN
            SET @Name = ( SELECT TOP 1
                                 Name
                          FROM   dbo.[netsqlazman_ApplicationGroups]()
         ...
         END
      ELSE 
         IF ( @SidWhereDefined = 2
              OR @SidWhereDefined = 3 ) --LDAP or LOCAL
            BEGIN
               SET @Name = ( SELECT SUSER_SNAME(@sid) )
            END
         ELSE 
            IF ( @SidWhereDefined = 4 ) --Database
               BEGIN
                  SET @Name = ( SELECT DBUserName
                                FROM   dbo.[netsqlazman_GetDBUsers](@StoreName, @ApplicationName, @sid, NULL) )
               END
   IF ( @Name IS NULL ) 
      BEGIN
         SET @Name = @sid
...
IF EXISTS ( SELECT   ItemId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ItemsHierarchyTable]
         (
           [ItemId],
           [MemberOfItemId]
         )
...
IF EXISTS ( SELECT   ItemId
   ...
   DELETE   FROM [dbo].[netsqlazman_ItemsHierarchyTable]
   WHERE    [ItemId] = @ItemId
            AND [MemberOfItemId] = @MemberOfItemId
ELSE 
   RAISERROR ( 'Application permission denied.', 16, 1 )
GO
...
RETURN
   ...
   FROM     dbo.[netsqlazman_ItemsHierarchyTable]
            INNER JOIN dbo.[netsqlazman_Items]() Items ON dbo.[netsqlazman_ItemsHierarchyTable].ItemId = Items.ItemId
GO
...
IF EXISTS ( SELECT   ItemId
            ...
            WHERE    ItemId = @ItemId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_ItemsTable]
   SET      BizRuleId = @BizRuleId
   WHERE    [ItemId] = @ItemId
            AND [ApplicationId] = @ApplicationId
ELSE 
   RAISERROR ( 'Application permission denied.', 16, 1 )
...
IF EXISTS ( SELECT   ApplicationGroupId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_ApplicationGroupMembersTable]
         (
           [ApplicationGroupId],
           [objectSid],
           [WhereDefined],
...
IF EXISTS ( SELECT   ItemId
            ...
            WHERE    ItemId = @ItemId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_ItemsTable]
   WHERE    [ItemId] = @ItemId
            AND [ApplicationId] = @ApplicationId
ELSE 
   RAISERROR ( 'Application permission denied.', 16, 1 )
...
AS SELECT   [netsqlazman_Stores].StoreId,
            [netsqlazman_Stores].Name,
            [netsqlazman_Stores].Description,
            [netsqlazman_StoreAttributes].StoreAttributeId,
            [netsqlazman_StoreAttributes].AttributeKey,
            [netsqlazman_StoreAttributes].AttributeValue
   FROM     dbo.[netsqlazman_Stores]() [netsqlazman_Stores]
            INNER JOIN dbo.[netsqlazman_StoreAttributes]() [netsqlazman_StoreAttributes] ON [netsqlazman_Stores].StoreId = [netsqlazman_StoreAttributes].StoreId
GO
...
AS SELECT   [netsqlazman_Applications].ApplicationId,
            [netsqlazman_Applications].StoreId,
            [netsqlazman_Applications].Name,
            [netsqlazman_Applications].Description,
            ApplicationAttributes.ApplicationAttributeId,
            ApplicationAttributes.AttributeKey,
            ApplicationAttributes.AttributeValue
   FROM     dbo.[netsqlazman_Applications]() [netsqlazman_Applications]
            INNER JOIN dbo.[netsqlazman_ApplicationAttributes]() ApplicationAttributes ON [netsqlazman_Applications].ApplicationId = ApplicationAttributes.ApplicationId
GO
...
RETURN
   ...
   FROM     dbo.[netsqlazman_ApplicationGroups]() ApplicationGroups
            INNER JOIN dbo.[netsqlazman_ApplicationGroupMembersTable] ON ApplicationGroups.ApplicationGroupId = dbo.[netsqlazman_ApplicationGroupMembersTable].ApplicationGroupId
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[netsqlazman_DeleteDelegate]
(
  @AUTHORIZATIONID INT,
  @OWNERSID VARBINARY(85)
)
AS 
DECLARE @ApplicationId INT
SELECT   @ApplicationId = Items.ApplicationId
FROM     dbo.[netsqlazman_Items]() Items
         INNER JOIN dbo.[netsqlazman_Authorizations]() Authorizations ON Items.ItemId = Authorizations.ItemId
WHERE    Authorizations.AuthorizationId = @AUTHORIZATIONID
IF @ApplicationId IS NOT NULL
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 1) = 1 
   DELETE   FROM dbo.[netsqlazman_AuthorizationsTable]
...
AS 
IF EXISTS ( SELECT   AuthorizationId
            FROM     dbo.[netsqlazman_Authorizations]()
            WHERE    AuthorizationId = @Original_AuthorizationId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_AuthorizationsTable]
   SET      [ownerSid] = @ownerSid,
            [ownerSidWhereDefined] = @ownerSidWhereDefined,
            [objectSid] = @objectSid,
            [objectSidWhereDefined] = @objectSidWhereDefined,
...
DECLARE @sub_members_cur CURSOR
DECLARE @OBJECTSID VARBINARY(85)
SELECT   @GROUPID = ApplicationGroupId,
         @GROUPTYPE = GroupType,
         @LDAPQUERY = LDapQuery
...
IF @GROUPTYPE = 0 -- BASIC
   BEGIN
	...
	-- Application Groups Members
      DECLARE nested_Application_groups_cur CURSOR LOCAL FAST_FORWARD
         FOR SELECT  objectSid,
                     IsMember
             FROM    dbo.[netsqlazman_ApplicationGroupMembersTable]
             WHERE   ApplicationGroupId = @GROUPID
            ...
            ELSE 
               BEGIN
                  ...
                  ELSE 
                     SET @NotMemberType = 0
               END
            EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] @NotMemberType, @MemberObjectSid, @NETSQLAZMANMODE,
               @LDAPPATH, @sub_members_cur OUTPUT
            FETCH NEXT FROM @sub_members_cur INTO @OBJECTSID
            WHILE @@FETCH_STATUS = 0
               BEGIN
                  INSERT   INTO @RESULT
...
AS 
IF EXISTS ( SELECT   ItemAttributeId
            FROM     dbo.[netsqlazman_ItemAttributes]()
            WHERE    ItemAttributeId = @Original_ItemAttributeId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_ItemAttributesTable]
   SET      [AttributeKey] = @AttributeKey,
            [AttributeValue] = @AttributeValue
   WHERE    [ItemAttributeId] = @Original_ItemAttributeId
ELSE 
...
IF EXISTS ( SELECT   ItemAttributeId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_ItemAttributesTable]
   WHERE    [ItemAttributeId] = @ItemAttributeId
ELSE 
   RAISERROR ( 'Application permission denied.', 16, 1 )
GO
...
AS 
IF EXISTS ( SELECT   StoreGroupMemberId
            FROM     dbo.[netsqlazman_StoreGroupMembers]()
            WHERE    StoreGroupMemberId = @StoreGroupMemberId )
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_StoreGroupMembersTable]
   WHERE    [StoreGroupMemberId] = @StoreGroupMemberId
ELSE 
...
IF EXISTS ( SELECT   StoreGroupMemberId
            FROM     dbo.[netsqlazman_StoreGroupMembers]()
            WHERE    StoreGroupMemberId = @Original_StoreGroupMemberId )
   AND dbo.[netsqlazman_CheckStorePermissions](@StoreId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_StoreGroupMembersTable]
...
AS 
IF EXISTS ( SELECT   BizRuleId
            FROM     dbo.[netsqlazman_BizRules]()
            WHERE    BizRuleId = @Original_BizRuleId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_BizRulesTable]
   SET      [BizRuleSource] = @BizRuleSource,
...
RETURN
   ...
   FROM     dbo.[netsqlazman_AuthorizationAttributesTable]
            INNER JOIN dbo.[netsqlazman_Authorizations]() AS Authorizations ON dbo.[netsqlazman_AuthorizationAttributesTable].AuthorizationId = Authorizations.AuthorizationId
GO
...
IF EXISTS ( SELECT   AuthorizationId
   ...
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 1) = 1 
   BEGIN
      INSERT   INTO [dbo].[netsqlazman_AuthorizationAttributesTable]
         (
           [AuthorizationId],
           [AttributeKey],
           [AttributeValue]
...
AS 
IF EXISTS ( SELECT   ApplicationGroupMemberId
            FROM     dbo.[netsqlazman_ApplicationGroupMembers]()
            WHERE    ApplicationGroupMemberId = @ApplicationGroupMemberId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   DELETE   FROM [dbo].[netsqlazman_ApplicationGroupMembersTable]
   WHERE    [ApplicationGroupMemberId] = @ApplicationGroupMemberId
ELSE 
...
IF EXISTS ( SELECT   ApplicationGroupMemberId
            FROM     dbo.[netsqlazman_ApplicationGroupMembers]()
            WHERE    ApplicationGroupMemberId = @Original_ApplicationGroupMemberId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 2) = 1 
   UPDATE   [dbo].[netsqlazman_ApplicationGroupMembersTable]
   SET      [objectSid] = @objectSid,
            [WhereDefined] = @WhereDefined,
            [IsMember] = @IsMember
   WHERE    [ApplicationGroupMemberId] = @Original_ApplicationGroupMemberId
...
AS SELECT   StoreGroupMembers.StoreGroupMemberId,
            StoreGroupMembers.StoreGroupId,
            StoreGroups.Name AS StoreGroup,
            dbo.[netsqlazman_GetNameFromSid](Stores.Name, NULL, StoreGroupMembers.objectSid,
                                             StoreGroupMembers.WhereDefined) AS Name,
            StoreGroupMembers.objectSid,
            CASE WhereDefined
              WHEN 0 THEN 'Store'
            ...
            END AS MemberType
   FROM     dbo.[netsqlazman_StoreGroupMembers]() StoreGroupMembers
            INNER JOIN dbo.[netsqlazman_StoreGroups]() StoreGroups ON StoreGroupMembers.StoreGroupId = StoreGroups.StoreGroupId
            INNER JOIN dbo.[netsqlazman_Stores]() Stores ON StoreGroups.StoreId = Stores.StoreId
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[netsqlazman_BuildUserPermissionCache]
(
  @STORENAME NVARCHAR(255),
  @APPLICATIONNAME NVARCHAR(255)
)
...
SELECT   Items.Name AS ItemName,
         Items_1.Name AS ParentItemName
FROM     dbo.[netsqlazman_Items]() Items_1
         INNER JOIN dbo.[netsqlazman_ItemsHierarchy]() ItemsHierarchy ON Items_1.ItemId = ItemsHierarchy.MemberOfItemId
         RIGHT OUTER JOIN dbo.[netsqlazman_Applications]() Applications
         INNER JOIN dbo.[netsqlazman_Stores]() Stores ON Applications.StoreId = Stores.StoreId
         INNER JOIN dbo.[netsqlazman_Items]() Items ON Applications.ApplicationId = Items.ApplicationId ON ItemsHierarchy.ItemId = Items.ItemId
WHERE    ( Stores.Name = @STORENAME )
         AND ( Applications.Name = @APPLICATIONNAME )

-- Item Authorizations
SELECT DISTINCT
         Items.Name AS ItemName,
         Authorizations.ValidFrom,
         Authorizations.ValidTo
FROM     dbo.[netsqlazman_Authorizations]() Authorizations
         INNER JOIN dbo.[netsqlazman_Items]() Items ON Authorizations.ItemId = Items.ItemId
         INNER JOIN dbo.[netsqlazman_Stores]() Stores
         INNER JOIN dbo.[netsqlazman_Applications]() Applications ON Stores.StoreId = Applications.StoreId ON Items.ApplicationId = Applications.ApplicationId
WHERE    ( Authorizations.AuthorizationType <> 0 )
...
WHERE    ( Stores.Name = @STORENAME )
         AND ( Applications.Name = @APPLICATIONNAME )
         AND Items.BizRuleId IS NOT NULL
SET NOCOUNT OFF
GO
...
AS SELECT   [netsqlazman_Authorizations].AuthorizationId,
            [netsqlazman_Authorizations].ItemId,
            dbo.[netsqlazman_GetNameFromSid]([netsqlazman_Stores].Name, [netsqlazman_Applications].Name,
                                             [netsqlazman_Authorizations].ownerSid,
                                             [netsqlazman_Authorizations].ownerSidWhereDefined) AS Owner,
            dbo.[netsqlazman_GetNameFromSid]([netsqlazman_Stores].Name, [netsqlazman_Applications].Name,
                                             [netsqlazman_Authorizations].objectSid,
                                             [netsqlazman_Authorizations].objectSidWhereDefined) AS Name,
            [netsqlazman_Authorizations].objectSid,
            CASE objectSidWhereDefined
              WHEN 0 THEN 'Store'
              WHEN 1 THEN 'Application'
              WHEN 2 THEN 'LDAP'
              WHEN 3 THEN 'Local'
              WHEN 4 THEN 'DATABASE'
            END AS SidWhereDefined,
            CASE AuthorizationType
              WHEN 0 THEN 'NEUTRAL'
              WHEN 1 THEN 'ALLOW'
              WHEN 2 THEN 'DENY'
              WHEN 3 THEN 'ALLOWWITHDELEGATION'
            END AS AuthorizationType,
            [netsqlazman_Authorizations].ValidFrom,
            [netsqlazman_Authorizations].ValidTo
   FROM     dbo.[netsqlazman_Authorizations]() [netsqlazman_Authorizations]
            INNER JOIN dbo.[netsqlazman_Items]() [netsqlazman_Items] ON [netsqlazman_Authorizations].ItemId = [netsqlazman_Items].ItemId
            INNER JOIN dbo.[netsqlazman_Applications]() [netsqlazman_Applications] ON [netsqlazman_Items].ApplicationId = [netsqlazman_Applications].ApplicationId
            INNER JOIN dbo.[netsqlazman_Stores]() [netsqlazman_Stores] ON [netsqlazman_Applications].StoreId = [netsqlazman_Stores].StoreId
GO
...
AS SELECT   [netsqlazman_Stores].StoreId,
            [netsqlazman_Applications].ApplicationId,
            [netsqlazman_ApplicationGroupMembers].ApplicationGroupMemberId,
            [netsqlazman_ApplicationGroupMembers].ApplicationGroupId,
            [netsqlazman_ApplicationGroups].Name AS ApplicationGroup,
            dbo.[netsqlazman_GetNameFromSid]([netsqlazman_Stores].Name, [netsqlazman_Applications].Name,
                                             [netsqlazman_ApplicationGroupMembers].objectSid,
                                             [netsqlazman_ApplicationGroupMembers].WhereDefined) AS Name,
            [netsqlazman_ApplicationGroupMembers].objectSid,
            CASE WhereDefined
              WHEN 0 THEN 'Store'
              WHEN 1 THEN 'Application'
            ...
            CASE IsMember
              ...
              WHEN 0 THEN 'Non-Member'
            END AS MemberType
   FROM     dbo.[netsqlazman_ApplicationGroupMembers]() [netsqlazman_ApplicationGroupMembers]
            INNER JOIN dbo.[netsqlazman_ApplicationGroups]() [netsqlazman_ApplicationGroups] ON [netsqlazman_ApplicationGroupMembers].ApplicationGroupId = [netsqlazman_ApplicationGroups].ApplicationGroupId
            INNER JOIN dbo.[netsqlazman_Applications]() [netsqlazman_Applications] ON [netsqlazman_ApplicationGroups].ApplicationId = [netsqlazman_Applications].ApplicationId
            INNER JOIN dbo.[netsqlazman_Stores]() [netsqlazman_Stores] ON [netsqlazman_Applications].StoreId = [netsqlazman_Stores].StoreId
GO
...
AS SELECT   [netsqlazman_Items].ItemId,
            [netsqlazman_Items].ApplicationId,
            [netsqlazman_Items].Name,
            [netsqlazman_Items].Description,
            CASE [netsqlazman_Items].ItemType
              WHEN 0 THEN 'Role'
              WHEN 1 THEN 'Task'
              WHEN 2 THEN 'Operation'
            END AS ItemType,
            Items_1.ItemId AS MemberItemId,
            Items_1.ApplicationId AS MemberApplicationId,
            Items_1.Name AS MemberName,
            Items_1.Description AS MemberDescription,
            CASE Items_1.ItemType
              WHEN 0 THEN 'Role'
              WHEN 1 THEN 'Task'
              WHEN 2 THEN 'Operation'
            END AS MemberType
   FROM     dbo.[netsqlazman_Items]() Items_1
            INNER JOIN dbo.[netsqlazman_ItemsHierarchy]() [netsqlazman_ItemsHierarchy] ON Items_1.ItemId = [netsqlazman_ItemsHierarchy].ItemId
            INNER JOIN dbo.[netsqlazman_Items]() [netsqlazman_Items] ON [netsqlazman_ItemsHierarchy].MemberOfItemId = [netsqlazman_Items].ItemId
            INNER JOIN dbo.[netsqlazman_Applications]() [netsqlazman_Applications] ON [netsqlazman_Items].ApplicationId = [netsqlazman_Applications].ApplicationId
GO
...
AS SELECT   [netsqlazman_Items].ItemId,
            ...
            [netsqlazman_Items].Name,
            [netsqlazman_Items].Description,
            [netsqlazman_Items].ItemType,
            [netsqlazman_BizRules].BizRuleSource,
            [netsqlazman_BizRules].BizRuleLanguage,
            [netsqlazman_BizRules].CompiledAssembly
   FROM     dbo.[netsqlazman_Items]() [netsqlazman_Items]
            INNER JOIN dbo.[netsqlazman_BizRules]() [netsqlazman_BizRules] ON [netsqlazman_Items].BizRuleId = [netsqlazman_BizRules].BizRuleId
GO
...
AS SELECT   [netsqlazman_Items].ItemId,
            ...
            CASE [netsqlazman_Items].ItemType
              ...
              WHEN 2 THEN 'Operation'
            END AS ItemType,
            [netsqlazman_ItemAttributes].ItemAttributeId,
            [netsqlazman_ItemAttributes].AttributeKey,
            [netsqlazman_ItemAttributes].AttributeValue
   FROM     dbo.[netsqlazman_Items]() [netsqlazman_Items]
            INNER JOIN dbo.[netsqlazman_ItemAttributes]() [netsqlazman_ItemAttributes] ON [netsqlazman_Items].ItemId = [netsqlazman_ItemAttributes].ItemId
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[netsqlazman_CheckAccess]
(
  @ITEMID INT,
  @USERSID VARBINARY(85),
  @VALIDFOR DATETIME,
  @LDAPPATH NVARCHAR(4000),
  @AUTHORIZATION_TYPE TINYINT OUTPUT,
  @NETSQLAZMANMODE BIT,
  @RETRIEVEATTRIBUTES BIT
)
AS ---------------------------------------------------
...
DECLARE ItemsWhereIAmAMember_cur CURSOR LOCAL FAST_FORWARD READ_ONLY
   FOR SELECT  MemberOfItemId
       FROM    dbo.[netsqlazman_ItemsHierarchyTable]
       WHERE   ItemId = @ITEMID
...
WHILE @@FETCH_STATUS = 0
   BEGIN
	-- Recursive Call
      EXEC dbo.[netsqlazman_CheckAccess] @PARENTITEMID, @USERSID, @VALIDFOR, @LDAPPATH, @PARENTRESULT OUTPUT,
         @NETSQLAZMANMODE, @RETRIEVEATTRIBUTES
...
DECLARE checkaccessonitem_cur CURSOR LOCAL FAST_FORWARD READ_ONLY
   FOR SELECT  AuthorizationType,
               AuthorizationId
       FROM    dbo.[netsqlazman_AuthorizationsTable]
       WHERE   ItemId = @ITEMID
               AND objectSid = @USERSID
               AND ( ValidFrom IS NULL
                     AND ValidTo IS NULL
                     OR @VALIDFOR >= ValidFrom
                     AND ValidTo IS NULL
                     OR @VALIDFOR <= ValidTo
                     AND ValidFrom IS NULL
                     OR @VALIDFOR BETWEEN ValidFrom AND ValidTo )
               AND AuthorizationType <> 0
               AND ( ( @NETSQLAZMANMODE = 0
                       AND ( objectSidWhereDefined = 2
                             OR objectSidWhereDefined = 4 ) )
                     OR ( @NETSQLAZMANMODE = 1
                          AND objectSidWhereDefined BETWEEN 2 AND 4 ) ) -- if Mode = Administrator SKIP CHECK for local Authorizations
	...
	--CHECK FOR DENY
      IF @PARTIAL_RESULT IS NOT NULL 
         BEGIN
            ...
            SELECT   @ITEM_AUTHORIZATION_TYPE = dbo.[netsqlazman_MergeAuthorizations](@ITEM_AUTHORIZATION_TYPE,
                                                                                      @PARTIAL_RESULT)
            IF @RETRIEVEATTRIBUTES = 1 
               INSERT   INTO #ATTRIBUTES_TABLE
                        SELECT   AttributeKey,
...
DECLARE usergroupsauthz_cur CURSOR LOCAL FAST_FORWARD READ_ONLY
   FOR SELECT  AuthorizationType,
               AuthorizationID
       FROM    dbo.[netsqlazman_AuthorizationsTable] Authorizations
               INNER JOIN #USERGROUPS AS usergroups ON Authorizations.objectSid = usergroups.objectSid
       WHERE   ItemId = @ITEMID
               AND ( ValidFrom IS NULL
                     AND ValidTo IS NULL
                     OR @VALIDFOR >= ValidFrom
...
DECLARE groups_authorizations_cur CURSOR LOCAL FAST_FORWARD READ_ONLY
   FOR SELECT  objectSid,
       ...
       FROM    dbo.[netsqlazman_AuthorizationsTable]
       WHERE   ItemId = @ITEMID
               AND objectSidWhereDefined BETWEEN 0 AND 1
               AND AuthorizationType <> 0
               AND ( ValidFrom IS NULL
                     AND ValidTo IS NULL
                     OR @VALIDFOR >= ValidFrom
...
--store groups members of type 'non-member'
            ...
            EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] 0, @GROUPOBJECTSID, @NETSQLAZMANMODE, @LDAPPATH,
               @members_cur OUTPUT -- non-members
            FETCH NEXT FROM @members_cur INTO @OBJECTSID
            WHILE @@FETCH_STATUS = 0
               BEGIN
                  INSERT   INTO @GROUPSIDMEMBERS
                  VALUES   ( @OBJECTSID )
                  FETCH NEXT FROM @members_cur INTO @OBJECTSID
               END
            CLOSE @members_cur
            ...
            IF EXISTS ( SELECT   *
                        FROM     @GROUPSIDMEMBERS
                        WHERE    objectSid = @USERSID )
               OR EXISTS ( SELECT   *
                           FROM     @GROUPSIDMEMBERS groupsidmembers
                                    INNER JOIN #USERGROUPS AS usergroups ON groupsidmembers.objectSid = usergroups.objectSid ) 
               BEGIN
	-- user is a non-member
            ...
            IF @ISMEMBER = 1 
               BEGIN
                  ...
                  EXEC dbo.[netsqlazman_GetStoreGroupSidMembers] 1, @GROUPOBJECTSID, @NETSQLAZMANMODE, @LDAPPATH,
                     @members_cur OUTPUT -- members
                  FETCH NEXT FROM @members_cur INTO @OBJECTSID
                  WHILE @@FETCH_STATUS = 0
                     BEGIN
                        INSERT   INTO @GROUPSIDMEMBERS
                  ...
                  IF EXISTS ( SELECT   *
                     ...
                     OR EXISTS ( SELECT   *
                                 FROM     @GROUPSIDMEMBERS groupsidmembers
                                          INNER JOIN #USERGROUPS usergroups ON groupsidmembers.objectSid = usergroups.ObjectSId ) 
                     BEGIN
		-- user is a member
                        SET @ISMEMBER = 1
                     END
	...
	-- if a member ... get authorization
            IF @ISMEMBER = 1 
               BEGIN
                  SET @AUTHORIZATION_TYPE = ( SELECT  dbo.[netsqlazman_MergeAuthorizations](@AUTHORIZATION_TYPE,
                                                                                            @PARTIAL_RESULT) )
                  SET @ITEM_AUTHORIZATION_TYPE = ( SELECT   dbo.[netsqlazman_MergeAuthorizations](@ITEM_AUTHORIZATION_TYPE,
                                                                                                  @PARTIAL_RESULT) )
                  IF @PKID IS NOT NULL
                     AND @RETRIEVEATTRIBUTES = 1 
                     INSERT   INTO #ATTRIBUTES_TABLE
                              SELECT   AttributeKey,
                                       AttributeValue,
                                       NULL
	...
	--application groups members of type 'non-member'
               ...
               EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] 0, @GROUPOBJECTSID, @NETSQLAZMANMODE, @LDAPPATH,
                  @members_cur OUTPUT -- non-members
               FETCH NEXT FROM @members_cur INTO @OBJECTSID
               WHILE @@FETCH_STATUS = 0
                  BEGIN
               ...
               IF @ISMEMBER = 1 
                  BEGIN
                     ...
                     EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] 1, @GROUPOBJECTSID, @NETSQLAZMANMODE,
                        @LDAPPATH, @members_cur OUTPUT -- members
                     FETCH NEXT FROM @members_cur INTO @OBJECTSID
                     WHILE @@FETCH_STATUS = 0
                        BEGIN
                           INSERT   INTO @GROUPSIDMEMBERS
...
INSERT   INTO #PARTIAL_RESULTS_TABLE
         SELECT   Items.ItemId,
                  ...
                  BizRules.BizRuleId,
                  BizRules.BizRuleSource,
                  BizRules.BizRuleLanguage
         FROM     dbo.[netsqlazman_ItemsTable] Items
                  LEFT OUTER JOIN dbo.[netsqlazman_BizRulesTable] BizRules ON Items.BizRuleId = BizRules.BizRuleId
...
AS 
IF EXISTS ( SELECT   AuthorizationAttributeId
            FROM     dbo.[netsqlazman_AuthorizationAttributes]()
            WHERE    AuthorizationAttributeId = @AuthorizationAttributeId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 1) = 1 
   DELETE   FROM [dbo].[netsqlazman_AuthorizationAttributesTable]
   WHERE    [AuthorizationAttributeId] = @AuthorizationAttributeId
ELSE 
   RAISERROR ( 'Application permission denied.', 16, 1 )
GO
...
IF EXISTS ( SELECT   AuthorizationAttributeId
            FROM     dbo.[netsqlazman_AuthorizationAttributes]()
            WHERE    AuthorizationAttributeId = @Original_AuthorizationAttributeId )
   AND dbo.[netsqlazman_CheckApplicationPermissions](@ApplicationId, 1) = 1 
   UPDATE   [dbo].[netsqlazman_AuthorizationAttributesTable]
   SET      [AttributeKey] = @AttributeKey,
            [AttributeValue] = @AttributeValue
   WHERE    [AuthorizationAttributeId] = @Original_AuthorizationAttributeId
ELSE 
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[netsqlazman_IsAMemberOfGroup]
(
  @GROUPTYPE BIT,
  @GROUPOBJECTSID VARBINARY(85),
  @NETSQLAZMANMODE BIT,
	...
	-- Get User Groups Sid
      ...
      WHILE ( @I < @USERGROUPSCOUNT )
         BEGIN
            ...
            INSERT   INTO @USERGROUPS ( objectSid )
            VALUES   ( @APP )
            SET @I = @I + 1
         END
   END
...
ELSE -- APPLICATON GROUP
   EXEC dbo.[netsqlazman_GetApplicationGroupSidMembers] 1, @GROUPOBJECTSID, @NETSQLAZMANMODE, @LDAPPATH,
      @member_cur OUTPUT
...
AS SELECT   dbo.[netsqlazman_AuthorizationView].AuthorizationId,
            dbo.[netsqlazman_AuthorizationView].ItemId,
            dbo.[netsqlazman_AuthorizationView].Owner,
            dbo.[netsqlazman_AuthorizationView].Name,
            dbo.[netsqlazman_AuthorizationView].objectSid,
            dbo.[netsqlazman_AuthorizationView].SidWhereDefined,
            dbo.[netsqlazman_AuthorizationView].AuthorizationType,
            dbo.[netsqlazman_AuthorizationView].ValidFrom,
            dbo.[netsqlazman_AuthorizationView].ValidTo,
            [netsqlazman_AuthorizationAttributes].AuthorizationAttributeId,
            [netsqlazman_AuthorizationAttributes].AttributeKey,
            [netsqlazman_AuthorizationAttributes].AttributeValue
   FROM     dbo.[netsqlazman_AuthorizationView]
            INNER JOIN dbo.[netsqlazman_AuthorizationAttributes]() [netsqlazman_AuthorizationAttributes] ON dbo.[netsqlazman_AuthorizationView].AuthorizationId = [netsqlazman_AuthorizationAttributes].AuthorizationId
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[netsqlazman_DirectCheckAccess]
(
  @STORENAME NVARCHAR(255),
  @APPLICATIONNAME NVARCHAR(255),
...
AS --Memo: 0 - Role; 1 - Task; 2 - Operation
SET NOCOUNT ON
DECLARE @STOREID INT
DECLARE @ApplicationId INT
DECLARE @ITEMID INT
...
FROM     dbo.[netsqlazman_Applications]() Applications
         INNER JOIN dbo.[netsqlazman_Items]() Items ON Applications.ApplicationId = Items.ApplicationId
         INNER JOIN dbo.[netsqlazman_Stores]() Stores ON Applications.StoreId = Stores.StoreId
WHERE    ( Stores.StoreId = @STOREID )
         AND ( Applications.ApplicationId = @ApplicationId )
         AND ( Items.Name = @ITEMNAME )
         AND ( @OPERATIONSONLY = 1
               AND Items.ItemType = 2
               OR @OPERATIONSONLY = 0 )
IF @ITEMID IS NULL 
   BEGIN
...
--------------------------------------------------------------------------------
      INSERT   INTO #ATTRIBUTES_TABLE
               SELECT   AttributeKey,
                        AttributeValue,
                        NULL
               FROM     dbo.[netsqlazman_StoreAttributesTable] StoreAttributes
                        INNER JOIN dbo.[netsqlazman_StoresTable] Stores ON StoreAttributes.StoreId = Stores.StoreId
               WHERE    Stores.StoreId = @STOREID
      INSERT   INTO #ATTRIBUTES_TABLE
               SELECT   AttributeKey,
                        AttributeValue,
                        NULL
               FROM     dbo.[netsqlazman_ApplicationAttributesTable] ApplicationAttributes
                        INNER JOIN dbo.[netsqlazman_ApplicationsTable] Applications ON ApplicationAttributes.ApplicationId = Applications.ApplicationId
...
FROM     dbo.[netsqlazman_Settings]
WHERE    SettingName = 'Mode'
...
EXEC dbo.[netsqlazman_CheckAccess] @ITEMID, @USERSID, @VALIDFOR, @LDAPPATH, @AUTHORIZATION_TYPE OUTPUT, @NETSQLAZMANMODE,
   @RETRIEVEATTRIBUTES
SELECT   *
FROM     #PARTIAL_RESULTS_TABLE
IF @RETRIEVEATTRIBUTES = 1 
   SELECT   *
...
ALTER TABLE [dbo].[netsqlazman_LogTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_Log] CHECK ( ( [LogType] = 'I'
                                 OR [LogType] = 'W'
                                 OR [LogType] = 'E' ) )
GO
ALTER TABLE [dbo].[netsqlazman_LogTable]
      CHECK CONSTRAINT [CK_Log]
GO
/****** Object:  Check [CK_Settings]    Script Date: 06/11/2009 17:45:33 ******/
ALTER TABLE [dbo].[netsqlazman_Settings]
      WITH CHECK
ADD
   CONSTRAINT [CK_Settings] CHECK ( ( [SettingName] = 'Mode'
                                      AND ( [SettingValue] = 'Developer'
                                            OR [SettingValue] = 'Administrator' )
                                      OR [SettingName] = 'LogErrors'
                                      AND ( [SettingValue] = 'True'
                                            OR [SettingValue] = 'False' )
                                      OR [SettingName] = 'LogWarnings'
                                      AND ( [SettingValue] = 'True'
                                            OR [SettingValue] = 'False' )
                                      OR [SettingName] = 'LogInformations'
                                      AND ( [SettingValue] = 'True'
                                            OR [SettingValue] = 'False' )
                                      OR [SettingName] = 'LogOnEventLog'
                                      AND ( [SettingValue] = 'True'
                                            OR [SettingValue] = 'False' )
                                      OR [SettingName] = 'LogOnDb'
                                      AND ( [SettingValue] = 'True'
                                            OR [SettingValue] = 'False' ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_Settings]
      CHECK CONSTRAINT [CK_Settings]
GO
...
ALTER TABLE [dbo].[netsqlazman_ApplicationGroupMembersTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_WhereDefinedNotValid] CHECK ( ( [WhereDefined] >= ( 0 )
                                                  AND [WhereDefined] <= ( 4 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationGroupMembersTable]
      CHECK CONSTRAINT [CK_WhereDefinedNotValid]
GO
/****** Object:  Check [CK_Items_ItemTypeCheck]    Script Date: 06/11/2009 17:45:36 ******/
ALTER TABLE [dbo].[netsqlazman_ItemsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_Items_ItemTypeCheck] CHECK ( ( [ItemType] >= ( 0 )
                                                 AND [ItemType] <= ( 2 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_ItemsTable]
      CHECK CONSTRAINT [CK_Items_ItemTypeCheck]
GO
...
ALTER TABLE [dbo].[netsqlazman_ApplicationGroupsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_ApplicationGroups_GroupType_Check] CHECK ( ( [GroupType] >= ( 0 )
                                                               AND [GroupType] <= ( 1 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationGroupsTable]
      CHECK CONSTRAINT [CK_ApplicationGroups_GroupType_Check]
GO
...
ALTER TABLE [dbo].[netsqlazman_ApplicationPermissionsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_ApplicationPermissions] CHECK ( ( [NetSqlAzManFixedServerRole] >= ( 0 )
                                                    AND [NetSqlAzManFixedServerRole] <= ( 2 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationPermissionsTable]
      CHECK CONSTRAINT [CK_ApplicationPermissions]
GO
/****** Object:  Check [CK_WhereDefinedCheck]    Script Date: 06/11/2009 17:45:38 ******/
ALTER TABLE [dbo].[netsqlazman_StoreGroupMembersTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_WhereDefinedCheck] CHECK ( ( [WhereDefined] = ( 0 )
                                               OR [WhereDefined] >= ( 2 )
                                               AND [WhereDefined] <= ( 4 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_StoreGroupMembersTable]
      CHECK CONSTRAINT [CK_WhereDefinedCheck]
GO
/****** Object:  Check [CK_StorePermissions]    Script Date: 06/11/2009 17:45:39 ******/
ALTER TABLE [dbo].[netsqlazman_StorePermissionsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_StorePermissions] CHECK ( ( [NetSqlAzManFixedServerRole] >= ( 0 )
                                              AND [NetSqlAzManFixedServerRole] <= ( 2 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_StorePermissionsTable]
      CHECK CONSTRAINT [CK_StorePermissions]
GO
/****** Object:  Check [CK_StoreGroups_GroupType_Check]    Script Date: 06/11/2009 17:45:39 ******/
ALTER TABLE [dbo].[netsqlazman_StoreGroupsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_StoreGroups_GroupType_Check] CHECK ( ( [GroupType] >= ( 0 )
                                                         AND [GroupType] <= ( 1 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_StoreGroupsTable]
      CHECK CONSTRAINT [CK_StoreGroups_GroupType_Check]
GO
/****** Object:  Check [CK_AuthorizationTypeCheck]    Script Date: 06/11/2009 17:45:40 ******/
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_AuthorizationTypeCheck] CHECK ( ( [AuthorizationType] >= ( 0 )
                                                    AND [AuthorizationType] <= ( 3 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      CHECK CONSTRAINT [CK_AuthorizationTypeCheck]
GO
/****** Object:  Check [CK_objectSidWhereDefinedCheck]    Script Date: 06/11/2009 17:45:40 ******/
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_objectSidWhereDefinedCheck] CHECK ( ( [objectSidWhereDefined] >= ( 0 )
                                                        AND [objectSidWhereDefined] <= ( 4 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      CHECK CONSTRAINT [CK_objectSidWhereDefinedCheck]
GO
/****** Object:  Check [CK_ownerSidWhereDefined]    Script Date: 06/11/2009 17:45:40 ******/
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_ownerSidWhereDefined] CHECK ( ( [ownerSidWhereDefined] >= ( 2 )
                                                  AND [ownerSidWhereDefined] <= ( 4 ) ) )
GO
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      CHECK CONSTRAINT [CK_ownerSidWhereDefined]
GO
/****** Object:  Check [CK_ValidFromToCheck]    Script Date: 06/11/2009 17:45:40 ******/
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      WITH CHECK
ADD
   CONSTRAINT [CK_ValidFromToCheck] CHECK ( ( [ValidFrom] IS NULL
                                              OR [ValidTo] IS NULL
                                              OR [ValidFrom] <= [ValidTo] ) )
GO
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      CHECK CONSTRAINT [CK_ValidFromToCheck]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_AuthorizationAttributesTable]
      CHECK CONSTRAINT [FK_AuthorizationAttributes_Authorizations]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationGroupMembersTable]
      CHECK CONSTRAINT [FK_ApplicationGroupMembers_ApplicationGroup]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ItemsTable]
      CHECK CONSTRAINT [FK_Items_Applications]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ItemsTable]
      CHECK CONSTRAINT [FK_Items_BizRules]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationGroupsTable]
      CHECK CONSTRAINT [FK_ApplicationGroups_Applications]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationAttributesTable]
      CHECK CONSTRAINT [FK_ApplicationAttributes_Applications]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationPermissionsTable]
      CHECK CONSTRAINT [FK_ApplicationPermissions_ApplicationsTable]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_StoreGroupMembersTable]
      CHECK CONSTRAINT [FK_StoreGroupMembers_StoreGroup]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ApplicationsTable]
      CHECK CONSTRAINT [FK_Applications_Stores]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_StoreAttributesTable]
      CHECK CONSTRAINT [FK_StoreAttributes_Stores]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_StorePermissionsTable]
      CHECK CONSTRAINT [FK_StorePermissions_StoresTable]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_StoreGroupsTable]
      CHECK CONSTRAINT [FK_StoreGroups_Stores]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_AuthorizationsTable]
      CHECK CONSTRAINT [FK_Authorizations_Items]
GO
...
GO
ALTER TABLE [dbo].[netsqlazman_ItemAttributesTable]
      CHECK CONSTRAINT [FK_ItemAttributes_Items]
GO
...
GO
GRANT SELECT ON [Basgosoft].[Membership] TO [NetSqlAzMan_Readers]
GO
...
IF CHARINDEX('Microsoft SQL Server 2005', REPLACE(@@VERSION, '  ', ' ')) = 1
   OR CHARINDEX('Microsoft SQL Server 2008', REPLACE(@@VERSION, '  ', ' ')) = 1 
   BEGIN
