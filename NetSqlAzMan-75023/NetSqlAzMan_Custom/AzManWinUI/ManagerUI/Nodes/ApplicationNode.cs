...
namespace AzManWinUI.Nodes
{
	internal class ApplicationNode : Basgosoft.ManagementConsoleLib.BaseNode
	{
		...
		protected override void renderNode()
		{
			...
			this.Text = String.Format("{0} ({1})", this.application.Name, fixedserverrole);
			this.ImageKey = ImageIndexes.ApplicationImgIdx;
			this.SelectedImageKey = ImageIndexes.ApplicationImgIdx;
			this.Tag = application;

			this.ListItemText = this.application.Name;
			this.FirstSubItemText = this.application.Description;
		...
		{
			listChildren.Add(new ApplicationGroupsNode(this.application, this.pttlstToolBar, this.ContextMenuStrip, this.pttvieTreeView, true, true, true));

			listChildren.Add(new ItemDefinitionsNode(this.application, this.pttlstToolBar, this.ContextMenuStrip, this.pttvieTreeView, true, true, true));

			listChildren.Add(new ItemAuthorizationsNode(this.application, this.pttlstToolBar, this.ContextMenuStrip, this.pttvieTreeView, true, true, true));
		}
		...
		{
			DialogResult dr = MessageBox.Show(String.Format(MultilanguageResource.GetString("Menu_Msg280") + "\r\n'{0}'", this.application.Name), MultilanguageResource.GetString("Menu_Msg270"), MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2);
