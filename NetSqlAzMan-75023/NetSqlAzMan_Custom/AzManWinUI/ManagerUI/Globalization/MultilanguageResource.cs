	...
	internal sealed class MultilanguageResource
	{
		...
		private static string GetStringInternal(string key)
		{
			...
			if (resourceManager == null)
				resourceManager = new ResourceManager(String.Format("AzManWinUI.Globalization.Resource_{0}", culture), Assembly.GetExecutingAssembly());
			string result;
			try
			{
		...
		private static string cultureSuffix(string cultureName)
		{
			...
			if (culture.StartsWith("english"))
				cultureString = "en";
			if (culture.StartsWith("italian"))
				cultureString = "it";
			if (culture.StartsWith("spanish"))
				cultureString = "es";
			if (culture.StartsWith("albanian"))
				cultureString = "sq";
			if (culture.StartsWith("russian"))
				cultureString = "ru";
		...
		internal static string cultureName(string cultureSuffix)
		{
			...
			if (culture.StartsWith("it"))
				cultureString = "Italian";
			if (culture.StartsWith("es"))
				cultureString = "Spanish";
			if (culture.StartsWith("sq"))
				cultureString = "Albanian";
			if (culture.StartsWith("ru"))
				cultureString = "Russian";
			return cultureString;
		}
