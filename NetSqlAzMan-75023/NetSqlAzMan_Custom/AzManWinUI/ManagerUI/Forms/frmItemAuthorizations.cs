...
namespace NetSqlAzMan.SnapIn.Forms
{
	public partial class frmItemAuthorizations : frmBase
	{
		...
		public frmItemAuthorizations() {
			...
			DataColumn dcLdapDomain = new DataColumn("LDAPDomain", typeof(string));
			dcLdapDomain.AllowDBNull = true;
			#endregion
			...
			#region ***PERSONALIZADO***
			dcLdapDomain.Caption = "LDAP Domain";
			#endregion
		...
		private void AddAuthorizationDataRow(IAzManAuthorization authorization) {
			...
			#region ***PERSONALIZADO***
			dr["LDAPDomain"] = authorization.LDAPDomain;
			#endregion
		...
		private DataRow AddLDapDataRow(GenericMember member, bool isAGroup) {
			...
			#region ***PERSONALIZADO***
			dr["LDAPDomain"] = member.LDAPDomain;
			#endregion
			this.dtAuthorizations.Rows.Add(dr);
				...
				if (toAdd != null) {
					foreach (DataRow dr in toAdd.Rows) {
						...
						IAzManAuthorization authorization = this.item.CreateAuthorizationCustom(
							 ...
							 (dr["LDAPDomain"] != DBNull.Value ? dr["LDAPDomain"].ToString() : null));
						#endregion
		...
		private void btnAddUsersAndGroupsLDAP_Click(object sender, EventArgs e) {
			AzManWinUI.LDAPServices.LDAPQueryClientUI _f = new AzManWinUI.LDAPServices.LDAPQueryClientUI(this.item.Application.Store.Storage);
			...
			foreach (ADObject o in res) {
				WhereDefined wd = WhereDefined.LDAP;
				GenericMember gm = new GenericMember(new SqlAzManSID(o.Sid), wd, o.LDAPDomain, AuthorizationType.Neutral, null, null) {
					Name = string.Format("{0}\\{1}", o.LDAPDomain, o.Name)
				};
