...
namespace AzManWebApiClientHelpers
{
	...
	{
		public LdapWebApiDomainProfilesHelper() : base()
		{
		}

		public LdapWebApiDomainProfilesHelper(string webApiUri) : base(webApiUri) {
		}
		...
		public async Task<Dictionary<string, IEnumerable<object>>> UpdateAsync(byte id, BSO sbo) {
			...
			using (var _h = Common.GetHttpClient(this.WebApiUri, true, Common.AcceptHeaderType.ApplicationJson)) {
		...
		public async Task<Dictionary<string, IEnumerable<object>>> RegisterAsync(BSO sbo) {
			var _return = new Dictionary<string, IEnumerable<object>>();
			string _requestUri = "api/LdapWebApiDomainProfile/";
			using (var _h = Common.GetHttpClient(this.WebApiUri, true, Common.AcceptHeaderType.ApplicationJson)) {
		...
		public async Task<Dictionary<string, IEnumerable<object>>> DeleteAsync(byte id, BSO sbo) {
			var _return = new Dictionary<string, IEnumerable<object>>();
